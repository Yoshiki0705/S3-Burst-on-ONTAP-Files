...
**このうちセキュリティスタイルの継承だけは、両側 FSx for ONTAP の構成で実測した**
（[継承の検証記録](verification/flexcache-security-style-inheritance.md)）。**Cache が
オンプレミス ONTAP の場合は未検証で、それ以外の前提も未確認のままである。**
