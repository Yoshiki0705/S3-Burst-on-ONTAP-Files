...
**パラメータは 21 個ありますが、書き換えが必要なのは 2 個だけです。** 残りは既定のまま動きます。
...
> **テンプレートを増やすときは、ボリューム名も一緒に登録してください。** 名前が一致しないと
> **バックアップは毎回報告されるのに一度も消えません**（報告で終わるので、消したつもりになります）。
> 2026-09-19 に 10 GiB の `smb_vol` のバックアップがこの形で残り続けました。
> **いまは全テンプレートからボリューム名を導出して照合するテストがあり**、登録漏れは
> 撤去時ではなく `make test` で落ちます。名前の一覧は
> [`scripts/sweep_after_teardown.py`](../../../scripts/sweep_after_teardown.py) にあります。
