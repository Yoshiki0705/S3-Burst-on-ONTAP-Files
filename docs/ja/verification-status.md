...
| NVMe/TCP のスループット | **未測定** | 同計画。**「対応している」は documented**（[Accessing your FSx for ONTAP data](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/supported-fsx-clients.html)。NVMe/TCP は第二世代かつ HA ペア 6 以下）。着手して namespace と subsystem までは作れたが、**map と host の追加が private CLI の passthrough で `invalid operation` になった**（POST が create に写るのに対し、この 2 つは `add` コマンド）。**修正済み・再実行前。** AL2023 で `nvme_tcp` と `dm_multipath` はロードされる（`nvme-cli` 2.13）が、**`/sys/module/nvme_core/parameters/multipath` はこのカーネルに存在しなかった** |
