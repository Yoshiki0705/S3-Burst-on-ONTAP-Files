...
| ヘルパーが 1 フローの上限を超える機構 | 検証済み | 同記録。読み取り中に数えると、**素マウントはマウントターゲット宛て TCP が 1 本、ヘルパー使用時は 5 本**で、すべて単一の `efs-proxy` プロセスが所有していた。**カーネルの NFS が話す相手は `127.0.0.1` なので `nconnect` は構造上届かず、代わりにプロキシが複数フローを張って同じ効果を出している。** 倍率 2.97 がフロー数 5 に比例しない理由は未確認 |
