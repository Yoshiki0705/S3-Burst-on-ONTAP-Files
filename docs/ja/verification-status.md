...
| ANA マルチパスでの NVMe/TCP | **未測定（この AMI では測れない）** | AL2023 のカーネルは **`# CONFIG_NVME_MULTIPATH is not set`** で、`/sys/module/nvme_core/parameters/multipath` も `modinfo nvme_core` の multipath パラメータも存在しない。**したがって AWS の手順が示す「`Y` が返れば成功」という検証はこの AMI では成立せず**、2 つのコントローラは 1 デバイスに統合されない。**「RHEL 9.3 前提の手順が AL2023 でそのまま通る」と読まない** |
