...
| ANA マルチパスでの NVMe/TCP | **未測定（測定に使った AMI では測れない）** | 測定に使ったカーネルは **`# CONFIG_NVME_MULTIPATH is not set`** で、`/sys/module/nvme_core/parameters/multipath` も `modinfo nvme_core` の multipath パラメータも存在しない。**したがって AWS の手順が示す「`Y` が返れば成功」という検証はこの AMI では成立せず**、2 つのコントローラは 1 デバイスに統合されない。**「RHEL 9.3 前提の手順が AL2023 でそのまま通る」と読まない。** ただし**「AL2023 では」と一般化しない**: 観測は `al2023-ami-kernel-default-x86_64` の SSM パラメータが 2026-09-12 / 13 に返した AMI に限り、**カーネル版を記録していなかった**（記録するよう修正済み） |
