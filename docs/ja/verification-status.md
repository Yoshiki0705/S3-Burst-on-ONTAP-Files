...
| EFS マウントヘルパーが一律に速いか | **未確認（再現しなかった）** | 同記録。Elastic + 300 GiB では 1,495.82 MB/s（素マウントの 3.0 倍）だが、Provisioned + 32 GiB では **498.61 MB/s** で素マウントと同じ。**2 回はスループットモードとテストファイルサイズの両方が違い、原因は切り分けていない。「ヘルパーは 3 倍」という一般化は取り下げる** |
