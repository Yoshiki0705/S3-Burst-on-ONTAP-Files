...
| EC2 の 1 クライアントあたりが 5 Gbps（約 625 MBps）であること | **documented（ブロックプロトコルでの実測は未測定）** | [Provisioning iSCSI for Linux](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/mount-iscsi-luns-linux.html) と [Provisioning NVMe/TCP for Linux](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/provision-nvme-linux.html) が、セッションを増やす手順の前置きに「Amazon EC2 single client maximum of 5 Gbps (~625 MBps)」と書き、配置グループ外の単一フロー 5 Gbps は [Amazon EC2 instance network bandwidth](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-instance-network-bandwidth.html) にある。**この環境のファイルプロトコルでの実測は 625 MBps ではなく、NFS 591.62（94.7%）・SMB 574.24（91.9%）だった。** iSCSI / NVMe/TCP で同じずれが出るかは**未測定**で、計画は[ブロックプロトコルの測定計画](verification/block-protocol-matrix-plan.md)にある |
| iSCSI と NVMe/TCP のスループット | **未測定** | 同計画。**「対応している」は documented**（[Accessing your FSx for ONTAP data](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/supported-fsx-clients.html)。NVMe/TCP は第二世代かつ HA ペア 6 以下）。**性能の数値はこのリポジトリに 1 点も無い。** 既存のファイル側の表に足せないのは、ブロックが両ノードの `iscsi_1` / `iscsi_2` を使う一方でファイル側の全バイトが `-01` の 1 ポートを通っていたためで、経路の本数が揃わない |
