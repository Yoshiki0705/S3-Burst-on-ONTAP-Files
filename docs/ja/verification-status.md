...
| 読み取りがキャッシュの温まり方だけで 45% 変わること | 検証済み | 同記録。同一設定・同一パラメータファイル・同一器具で 3,551.18 と 5,148.56。**両行とも多重度の検算（IOPS × 応答時間 = 512）を通る。** 差は直前に何を読んだかだけ。**この測定はキャッシュ状態をそろえていないので、複数接続の行は上下 45% の幅で読む** |
...
| NFS のバージョンがスループットを変えるか | 検証済み（変えないことを確認） | 同記録。`vers=` 以外を変えず 4 版。v3 5,342.23 / v4.0 4,803.59 / v4.1 5,148.56 / v4.2 5,116.81 MB/s で **幅は 11%**、上のキャッシュ由来 45% より小さい。**差が出たのは裾で、v4.1 / v4.2 の標準偏差は v3 / v4.0 の 1/16**（21.5・25.7 対 348.9・358.7）、最大応答も 3〜6 分の 1 |
