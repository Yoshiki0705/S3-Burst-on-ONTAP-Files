...
|---|---|---|
| ネットワーク I/O | throughput capacity | 段ごと。128 MBps の段でベースライン 150 MBps |
| ディスク I/O（ファイルサーバー側） | throughput capacity | 段の値と同じ（128 の段で 128、2048 の段で 2048） |
...
1. **SSD IOPS が段に見合っているか。** SSD 1 TiB で `AUTOMATIC` のままだと、段をいくら上げても
   ...
   **286.1 → 2042.1 MB/s（7.14 倍）**になった。**768 MBps は容量が決める天井ではなく、既定の
   ...
   > 読み取りは**バイトの 98.5〜99.9% がディスクに行っていなかった。** 280 GiB はこの段の
   > キャッシュ 238 GiB を 18% 超えただけで、「キャッシュに入らない」というラベルが甘かった。
   > 再測でも 2042.1 は 2667.2 になり、**「段の 99.7%」という一致は偶然だった。**
   > **IOPS を上げる前に、下の 3 番を確認すること。**
2. **読み取り主体か書き込み主体か。** 書き込み主体なら上限は 750 MBps 側で決まり、段を
   750 MBps 相当より上げても効かない

[実測記録](../../verification/throughput-iops-concurrency.md)で 2 段を測った結果、
**128 MBps の段では段が上限として効き（129.5 MB/s、101%）、2048 MBps の段では効かなかった**
（497.1 MB/s、書き込み上限 750 の 66%）。段を 16 倍にして書き込みは 3.8 倍である。
...
**2 回独立に再現した所見はこれだけなので、段を買い増す前にマウントを確認する。**
...
違っている。** 実際に「段のディスクスループットの 99.7%」という一致は 1 日で崩れた。
...
全体で 129.6 MB/s でした。低下も枯渇もありません。**購入した段が最初から最後まで上限です。**
