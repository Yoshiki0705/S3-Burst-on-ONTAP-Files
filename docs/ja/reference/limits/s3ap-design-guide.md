...
**「プロビジョンドスループット」を単一の上限として扱うと外す。** 効く上限は 3 つあり、
実際の上限はその最小値である（[性能仕様](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/performance.html)）。

| 上限 | 決まり方 | ap-northeast-1 / 第一世代 Single-AZ の値 |
|---|---|---|
| ネットワーク I/O | throughput capacity | 指定値ごと。128 MBps の構成でベースライン 150 MBps |
| ディスク I/O（ファイルサーバー側） | throughput capacity | 指定値と同じ（128 の構成で 128、2048 の構成で 2048） |
| **ディスク I/O（SSD 側）** | **プロビジョニングした SSD IOPS** | **768 MBps / TiB**、SSD IOPS は 3,072 / TiB（**SSD IOPS が `AUTOMATIC` のとき**） |

さらに**読み取りと書き込みで上限が違う**。HA ペアあたりの上限は次のとおりである。

| | 読み取り | 書き込み |
|---|---|---|
| 第一世代 Single-AZ（ap-northeast-1 など） | 2,048 MBps | **750 MBps** |
| 第一世代 Multi-AZ（同） | 2,048 MBps | 1,300 MBps |

**書き込みが低いのは、セカンダリのファイルサーバーへ複製されるためである。** 1 回の書き込みが
2 倍のネットワークスループットを使うと明記されている。

設計時に確認するのは次の 2 点になる。

1. **SSD IOPS が指定値に見合っているか。** SSD 1 TiB で `AUTOMATIC` のままだと、指定値をいくら上げても
   ディスクスループットは 768 MBps で止まる。外す方法は 2 つあり、SSD を 2.67 TiB 以上にするか、
   **SSD IOPS を `USER_PROVISIONED` にする。** 後者を実測すると、280 GiB の読み取りが
   **286.1 → 2042.1 MB/s（7.14 倍）**になった。**768 MBps は容量が決める天井ではなく、既定の
   IOPS 水準を MB/s で言い直したもの**である

   > **この 7.14 倍は観測であって、機構は未特定である。** 後日 CloudWatch を見ると、速い側の
   > 読み取りは**バイトの 98.5〜99.9% がディスクに行っていなかった。** 280 GiB はこの構成の
   > キャッシュ 238 GiB を 18% 超えただけで、「キャッシュに入らない」というラベルが甘かった。
   > 再測でも 2042.1 は 2667.2 になり、**「指定値の 99.7%」という一致は偶然だった。**
   > **IOPS を上げる前に、下の 3 番を確認すること。**
2. **読み取り主体か書き込み主体か。** 書き込み主体なら上限は 750 MBps 側で決まり、指定値を
   750 MBps 相当より上げても効かない

[実測記録](../../verification/throughput-iops-concurrency.md)で 2 つの指定値を測った結果、
**128 MBps の構成では指定値が上限として効き（129.5 MB/s、101%）、2048 MBps の構成では効かなかった**
（497.1 MB/s、書き込み上限 750 の 66%）。指定値を 16 倍にして書き込みは 3.8 倍である。

**3 つめのレバーはクライアント側にあり、無料で、この検証でいちばん確実に効いた。** Linux の NFS
マウントは既定でサーバーごとに TCP 接続を 1 本しか張らず、VPC 内の単一フローは 5 Gbps 付近で
止まる。同じマウントでストリーム数を 1 / 4 / 8 と振った読み取りは、**2 つのファイルシステムで
別の日に測って次のとおりである。**

| ファイルシステム | 1 ストリーム | 4 ストリーム | 8 ストリーム |
|---|---|---|---|
| 1 つめ（既定マウント） | 588.1 | 589.9 | 589.7 |
| 2 つめ（既定マウント） | 613.1 | 618.7 | 618.7 |
| 2 つめ（`nconnect=16`） | 1140.6 | 2904.6 | **3062.8** |

**既定のマウントはストリーム数を 8 倍にしても 1% 未満しか動かない。** 応答しているのが
クライアントだからである。`nconnect=16` を付けると 8 ストリームで **4.95 倍**になる。
**2 回独立に再現した所見はこれだけなので、指定値を買い増す前にマウントを確認する。**

### 自環境で測るときに先に外す 5 つの既定値

上の数値は、いずれも既定値のせいで一度は間違えて測ったものである。**5 つとも「もっともらしい
数値」を出すので、結果を見ただけでは気づけない。** 自環境で測るなら先に外す。

| 既定値 | 何が起きるか | 気づき方 | 対処 |
|---|---|---|---|
| Linux NFS の TCP 接続は 1 本 | 約 590 MB/s で止まり、**ストリーム数に反応しない** | 1 / 4 / 8 で値が動かない | `nconnect=16` |
| `dd if=/dev/zero` | ゼロブロックはディスクに行かず、**上限の 4 倍で返る** | 公称上限を超える | 非圧縮データ（`openssl enc -aes-256-ctr` で生成する） |
| ボリュームのインライン効率化が有効 | 圧縮可能・同一のペイロードが潰れる。**読み取りでは 4 倍、書き込みでは 5% 未満** | `space_savings.dedupe_percent` が高い | 測定中だけオフ。**データを書く前に切る**（書いた直後はバックグラウンド処理が走っていて切れない） |
| `DiskIopsConfiguration: AUTOMATIC` | 3 IOPS/GiB で IOPS 律速になり、スループット上限に届かない | MB/s ÷ IOPS が IO サイズとして不自然に小さい | `USER_PROVISIONED` |
| **キャッシュ量を「少し」超えた読み取り** | キャッシュから返る割合が支配し、**ディスク経路を測っていない** | CloudWatch の `DiskReadBytes` ÷ `DataReadBytes` が数 % 以下 | キャッシュの **2 倍以上**を一度に読む |

**測定値は公称上限に対する割合として書く。** 割合が 100% を大きく外れたら、数値ではなく測定を
疑う。**低すぎる値も、高すぎる値と同じくらい疑う対象である**（上の例では 37% と 400% の両方が
測定の誤りだった）。

**そして上限にきれいに当たった数値ほど、別の日に測り直すこと。** 同一環境・同一条件の再現性は
**0.2% 以内**だった（4 組の複製）。**再測で 30% 動いたなら、それはばらつきではなく条件の理解が
違っている。** 実際に「指定値のディスクスループットの 99.7%」という一致は 1 日で崩れた。

### ONTAP が飽和しているかを見る手段

`fsxadmin` の資格情報では ONTAP のノードレベル統計に届かない（`/api/cluster/nodes?fields=statistics`
は 0 件、`private/cli` の該当エンドポイントは `API not found`）。
**CloudWatch の `AWS/FSx` に 33 個のメトリクスがあり、そこで分かる。** <!-- allow:naming CloudWatch の名前空間名 -->

| 見たいこと | メトリクス |
|---|---|
| ONTAP が飽和しているか | `CPUUtilization` |
| 読み取りがディスクから来ているか | `DiskReadBytes` ÷ `DataReadBytes` |
...
実測では **S3 Access Point 経由で 417 MB/s のとき CPU 21〜24%、NFS で 800 MB/s のときも 18〜23%**
だった。**同じ CPU で 2 倍のスループットが出ているので、経路差の代価は ONTAP の中ではない。**
...
どちらも自分のワークロードで測る必要があり、スループットの指定値からは導出できない。
...
#### SSD IOPS より先に来るリクエストレートの頭打ち

**小さいオブジェクトを大量に扱う設計では、SSD IOPS を上限として計算すると外します。**
S3 Access Point 経由のリクエストレートを実測した結果は次のとおりです（4 KiB と 64 KiB、
128 MBps、SSD IOPS 3,072、各点 20 秒、503 は全点 0 件）。

| 並列度 | 4 KiB 書き込み | 4 KiB 読み取り | 64 KiB 書き込み |
|---|---|---|---|
| 16 | 153.9 | 330.3 | 171.2 |
| 64 | **415.3** | **630.3** | **415.9** |
| 256 | 420.8 | 594.0 | 404.4 |

単位 req/s。**書き込みは約 420 req/s、読み取りは約 600 req/s で頭打ちになり、SSD IOPS 3,072 の
14〜20% にしか届きません。** サイズをほぼ問わず（4 KiB と 64 KiB で 415.3 対 415.9）、
並列度を 64 から 256 に上げても増えません。

律速しているのは S3 API のリクエスト経路で、ストレージの IOPS ではありません。したがって
**SSD IOPS の追加プロビジョニング（$0.0204/IOPS-Mo）は、S3 経由のリクエストレートを上げません。**
増やす前に、どちらで詰まっているかを確認してください。

#### 複数クライアントで総量は増えない

**購入した容量は共有されます。** 同一種別のホストを 1 台から 4 台まで増やして測った合計です
（8 MiB、並列度 16、c5n.2xlarge）。

| 位相 | 1 台 | 2 台 | 3 台 | 4 台 | 4 台/1 台 |
|---|---|---|---|---|---|
| 書き込み | 129.4 | 130.9 | 131.5 | 131.0 | **1.01x** |
| 読み取り | 480.9 | 600.7 | 598.6 | 612.3 | 1.27x |

単位 MB/s（合計）。**書き込みは 4 台まで完全に平ら**で、1 台あたりは 129.4 → 65.3 → 43.5 → 33.0 と
台数で割った値になります。**容量計画はクライアント台数ではなくファイルシステム単位で行ってください。**

読み取りの 1.27 倍は上限が動いたのではありません。**1 台（480.9）ではこの上限に届かず**、
2 台で 600.7 に達してそこから平らになります。上限は約 600 MB/s です。8 vCPU のホスト 1 台では
並列度 16 の TLS 処理が先に頭打ちになるため、**上限を確認するには複数台が必要でした。**

（対照として、同じ測定を Amazon S3 に対して行うと合計が 4.22 倍（書き込み）と 4.10 倍（読み取り）
になり、1 台あたりは変わりません。上限の種類が違うためで、詳細は
[実測記録](../../verification/throughput-iops-concurrency.md#クライアント台数を-1-から-4-まで上げたとき)にあります。）

#### 収集層と配布層による同じ容量の取り合い

**この節の冒頭の主張には実測があります。** 10 分間の書き込み測定を 2 回行い、片方だけ途中で
別リージョンの FlexCache が同じ origin を読み始めました。

| 区間 | 他の負荷あり | 他の負荷なし |
|---|---|---|
| 0〜510 秒 | 127.8〜130.3 MB/s | 127.8〜130.6 MB/s |
| 510〜600 秒 | **112.7 → 66.8 → 66.0** | 129.2 → 130.0 → 130.0 |

**S3 Access Point 経由の書き込みが約半分になりました。** 2 回の測定の差は同時実行された
FlexCache の充填だけです。Origin クラスタに直接読み書きするクライアントがいる場合に
スループット分配を考える理由が、これです。

#### バースト機構は書き込みに関与しない

10 分間の連続書き込みを 30 秒ごとに区切ると、**20 区間すべてが 127.8〜130.6 MB/s** に収まり、
全体で 129.6 MB/s でした。低下も枯渇もありません。**購入した指定値が最初から最後まで上限です。**

短い測定でバースト分を測ってしまう心配はこの構成では要りませんが、**逆に「しばらく待てば速く
なる」も起きません。**

#### この手順を実行した結果

[スループット実測記録](../../verification/throughput-iops-concurrency.md)で上の手順を
実行した（2026-09-01、ap-northeast-1、128 MBps、8 MiB）。式の当たり方は次のとおり。

| 指定値 | 内容 |
|---|---|
| 並行度 1 の所要時間 | p50 331.5 ms、21.8 MB/s |
| 式が出す最大並行度 | 128 ÷ 25.3 ≈ **5** |
| 実測で飽和が始まる位置 | 並行度 4（97.5 MB/s）と 16（129.5 MB/s）の間 |
| 実測で頭打ちになる位置 | 並行度 16（129.5 MB/s） |

**式は膝の位置をほぼ当て、頭打ちに必要な並行度は少なめに出る。** 出発点としては使えるが、
式の値で設計を止めると指定値を使い切れない。

**1 リクエストあたり約 330 ms の固定費はサイズにほぼ依存しない**（1 MiB で 363.7 ms、
8 MiB で 331.5 ms）。したがって小さいオブジェクトほど 1 リクエストあたりの消費帯域が小さく、
必要な並行度は増える。1 MiB では並行度 64 まで伸び続けた。

### スループット飽和時の挙動

**飽和しても `SlowDown` (503) は返らなかった。** 128 MBps と 2048 MBps の両方の指定値で、
1 MiB / 8 MiB × 並行度 1/4/16/64 の**全 32 点で 503 は 0 件**である
（[実測記録](../../verification/throughput-iops-concurrency.md)、リトライを無効化して計数）。
書き込みは指定値で頭打ちになっており、飽和していたことは確かである。

**観測された飽和の現れ方は待ち行列である。** 8 MiB で並行度を 16 から 64 に上げると、
スループットは 129.5 から 127.5 MB/s へ動かない一方、p50 は 985.7 ms から 3885.8 ms に
伸びた。**帯域は増えず、遅延だけが並行度に比例して伸びる。**

設計上の含意は 2 つある。

- **503 の発生率を飽和の検知に使えません。** 上の手順 2 は記録項目としては正しいものの、
  この環境では 503 が出ないまま飽和に達しました。**飽和は p99 の伸びで検知します。**
- **タイムアウトが先に来ます。** 遅延が伸び続けるので、クライアントの read timeout が
  実質的な上限として先に効きます。

`SlowDown` が原理的に返らないという主張ではありません。この 32 点では観測されなかった、
という意味です。返る場合に備えた実装は引き続き必要で、exponential backoff（base: 1 秒、
max: 30 秒）と boto3 の `adaptive` retry mode が推奨です。**ただし計測するときは retry を
無効にします。** `adaptive` は 503 を内部で吸収するため、有効なままでは絞られた状態が
「遅い」に見えます。
...
| オブジェクト全体が 50 GiB を超える | S3 AP の**アップロード**上限。判定は転送後なので、送る前にクライアント側で検証する。**取得側には同じ上限が無い**（[上限値](s3-access-point.md#上限がアップロード側にしかないこと)） |
...
試算は仮定に依拠している。次の 3 つがずれると結論が変わる。
...
### 階層化できない Cache と、作業セット増加分の SSD 化
...
### write-back で Cache 側から変更できる属性の限定
