...
| **ダウンロードするオブジェクト全体** | **上限なし（50 GiB を超えても取得できる）** | ドキュメント記載 | 下の非対称性の節 |
...
> **ただし当プロジェクトでは未測定である。** ドキュメントに書いてあることと、
> 50 GiB を超えるファイルを NFS で作って S3 で `GetObject` したことは別である。
> **段階は「ドキュメント記載」であって「検証済み」ではない。**
