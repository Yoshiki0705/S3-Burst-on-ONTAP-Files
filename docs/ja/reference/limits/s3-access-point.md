...
| 項目 | 値 | 段階 | 備考 |
|---|---|---|---|
| 単一 `PutObject` | 5 GiB | 検証済み | 姉妹リポジトリでの実測。実測は 2 進接頭辞（5,368,709,120 バイト）。**この値を書いた公開ページは特定できていない** |
| `UploadPart` 1 パート | 5 GiB | 検証済み | 同上 |
| **アップロードするオブジェクト全体** | 50 GiB | **ドキュメント記載 + 検証済み** | 判定は `CompleteMultipartUpload` の時点で行われるため、全ペイロードの転送後に失敗する。クライアント側で先に検証する |
| **ダウンロードするオブジェクト全体** | **上限なし（50 GiB を超えても取得できる）** | **検証済み** | 50 GiB + 1 バイトを全体 GET できた。下の非対称性の節 |
...
**50 GiB はアップロードの上限で、取得の上限ではない。** この構成では収集が S3 API、利用が
NFS / SMB なので、素直に読むと関係が薄い。**効いてくるのは逆向きに読んだときである。**

| 経路 | 50 GiB を超えるオブジェクト |
|---|---|
| S3 API で書く | **作れない。** `CompleteMultipartUpload` で失敗する |
| NFS / SMB で書いたものを S3 API で読む | **取得できる** |

**つまり 50 GiB を超えるファイルは、ファイル側で作れば S3 側から読める。** 収集を S3 に寄せる
設計でも、大きいファイルだけファイル側で作るという逃げ道が documented である。

**2026-09-11 に実測した。段階は検証済みへ上がった。**

| 測定 | 結果 |
|---|---|
| `HeadObject` | `ContentLength: 53687091201`（50 GiB + 1 バイト）が返る。`StorageClass: FSX_ONTAP`、`ServerSideEncryption: aws:fsx` |
| Range GET 先頭 1 MiB | 成功。`ContentRange: bytes 0-1048575/53687091201` |
| Range GET 末尾 1 MiB | 成功。`ContentRange: bytes 53686042625-53687091200/53687091201` |
| **全体 GET** | **成功。53,687,091,201 バイトを 537 秒**（約 100 MB/s、指定値 128 MBps の 78%） |

**コントロールを同一セッションに置いた。** 1 GiB のファイルを同じ手順で取得し、16 秒で成功
（1,073,741,824 バイト）。**経路が生きていることを示さずに大きい側だけ測ると、失敗したときに
「50 GiB 超だから」と「経路が壊れていた」を区別できない。**

測定条件: 2026-09-11、ap-northeast-1、第一世代 SINGLE_AZ_1、**128 MBps**、SSD 1,024 GiB、
ボリューム 70 GiB / UNIX、NFSv3（`rsize=wsize=65536`、実効値を `/proc/mounts` で確認）、
クライアント t3.small、S3 AP は `NetworkOrigin=VPC` / `FileSystemIdentityType=UNIX`。

> **ファイルは `truncate` で作った疎ファイルである**（`blocks=0`）。**判定には影響しない**:
> S3 から見えるサイズは apparent size で決まり、GET はゼロブロックを返す。**ただし 537 秒という
> 所要時間をディスク読み取りの性能として引用してはいけない。** ディスクを読んでいない。
>
> **ONTAP のバージョンを記録し忘れた。** 環境を消したあとで気づいたので取れない。
> 次に同じ測定をするときは、作成直後に `/api/cluster?fields=version` を読む。

**2026-09-11 に確認したこと**: この記載は **GiB 表記**である。以前は「50 GB」表記で、
2 進値との食い違いをベンダーへ照会し「修正中」と回答を得ていた。**修正は反映済みである。**
一方 **単一 `PutObject` の 5 GiB を書いた公開ページは、同日の調査では見つけられなかった。**
「見つけられない」は「無い」ではないので、こちらは実測のみを根拠として残す。
...
| アクセスポイントポリシーのサイズ | ドキュメント上の上限は 20 KB。**判定は正規化後の文書に対して行われる** | ドキュメント記載 / **別環境での実測** | 24,620 B は受理、24,861 B は `MalformedPolicy: Normalized policy document exceeds the maximum allowed size`。境界は書き方で動くので**手元の JSON のバイト数を予算にできない**。Amazon FSx の API がフィールドとして受け付ける 200,000 文字は実効上限ではない <!-- allow:naming - サービスファミリーの API 名。FSx for ONTAP の略記ではない -->（[実測](https://github.com/Yoshiki0705/FSx-for-ONTAP-Adoption-Playbook/blob/main/docs/ja/domains/security-governance/notes/access-point-authorization-layers.md#正規化後で判定されるポリシーサイズの上限)。ONTAP 9.18.1P3D1、2026-08-17〜18、`ap-northeast-1`。**このリポジトリでは測っていない**） |
...
| 機能 | 状態 |
|---|---|
| イベント通知 | 対象外。ポーリング、または ONTAP ネイティブ監査ログを起点にする。**FPolicy は代替にならない**（AP 経由の書き込みは通知されない。実測 2026-08-26 / ONTAP 9.18.1P3D1） |
| ライフサイクル | 対象外 |
