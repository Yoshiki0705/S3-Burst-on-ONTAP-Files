...
| ファイル → S3 コピー | 対象が確定していて、**鮮度要件がジョブ間隔で足りる**（スケジュールの下限は転送サービス側で決まる。[DataSync は 1 時間より短くできない](https://docs.aws.amazon.com/datasync/latest/userguide/task-scheduling.html)） | ジョブ間隔より短い鮮度が要る、容量が大きい、削除の伝播が要る |
...
### ファイルを正本とし、分析や配布のために S3 へコピーする構成
...
### S3 を正本とし、ファイルアクセスをゲートウェイや FUSE で代替する構成
...
| 代償 | 内容 |
|---|---|
| 正本の不在 | 差分が出たときに、どちらが正しいかを決める規則が組織に存在しないことが多い |
...
| 代償 | 内容 |
|---|---|
| 利用側の前提 | マウントヘルパー (`amazon-efs-utils` に含まれる) が必要。対応コンピュートは EC2、Lambda、EKS、ECS（**EC2 以外はドキュメント記載のみで、このリポジトリでは[未検証](../../verification-status.md)**） |
...
| 競合 | 双方が同じファイルを変更するとバケットが正本になり、ファイルシステム側は lost and found へ移される |
| アーカイブ系ストレージクラス | Glacier 系と Intelligent-Tiering のアーカイブ層はファイルシステムから読めない |
| 設置場所 | AWS 上のコンピュートが前提。オンプレミスの装置からは使えない |
| 費用の形 | 正本のバケット料金に、高性能ストレージと読み書きの課金が加わる。しきい値と有効期限の設定で大きく動く |
