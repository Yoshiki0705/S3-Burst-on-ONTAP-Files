...
| Windows 識別情報 | AD 参加は必須ではない。ドメインが利用できない場合は workgroup モードの CIFS サーバー（[公式手順](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/smb-server-workgroup-setup.html)。NTLM のみ、Kerberos 非対応）。**REST での作成を実測した（2026-09-19、ONTAP 9.18.1P6）**: `POST /api/protocols/cifs/services` に `{"svm":{"name":...},"name":"SMBSRV1","workgroup":"WORKGROUP","enabled":true}` で、AD なしに `enabled=true` の CIFS サーバーが立つ。**同時に `is_multichannel_enabled` の既定が `false` であることも確認した。** **そのうえで Multichannel が AD なしに成立することを実測した（2026-09-20、ONTAP 9.18.1P6、第二世代 1,536 MBps）**: サーバー側で `is_multichannel_enabled=true`（`max_connections_per_session=32`）にして Windows Server 2022 からローカル ONTAP ユーザーの NTLM で接続すると、**ダイアレクトは SMB 3.1.1、`CurrentChannels` は 4** になった（RSS 対応 ENA 1 枚、`ConnectionCountPerRssNetworkInterface=4`）。**つまり Multichannel の測定にディレクトリは要らない。** ローカルユーザー名は SVM 名ではなく **CIFS サーバー名**で前置される（`SMBWG1\bench`）ため、`v18c-smb-svm\bench` 形式でグループに追加すると `Failed to resolve name` で失敗する。前提として **TCP 445 の開放が別に必要**で、ディレクトリの有無に紐付けてはいけない **AD 参加を選んだ場合は**、すべてのデータ操作に AD ドメインコントローラーへの到達性が必要になり、`HeadBucket` は AD 到達不能でも成功するため疎通確認に使えない |
...
| 監視 | **FPolicy は収集層の経路を見ない。** S3 Access Point 経由の操作は通知されず、`mandatory` 指定の同期ポリシーでも遮断されない。ONTAP ネイティブ監査ログは記録する（`Source=HTTP` / `Source=S3`）。ARP は AP 経由の書き込みを検知する。FPolicy の通知を起点にする検知・DLP・遮断は、この経路には効かない（[実測](https://github.com/Yoshiki0705/FSx-for-ONTAP-S3AccessPoints-Serverless-Patterns/blob/main/docs/errata-fpolicy-s3ap-coverage.md)） |
| ONTAP 機能との併用 | 実測（2026-08-26 / 9.18.1P3D1）では、S3 Access Point 経路で **Qtree は使え、Quota は S3 の PUT を拒否し、FlexClone はボリューム単位（クローン自身にアクセスポイントを取り付けられる）とファイル単位（クローンもアクセスポイント経由のオブジェクトとして見える）の両方が動き、FlexGroup にもアクセスポイントを取り付けられ、その FlexGroup のクローンも作れた**。ファイル単位クローンは 202 を返すがジョブ記録が残らないため、**失敗が観測できない**（ボリューム作成・ボリュームクローンのジョブは取得できるので対照が取れている）。NetApp が ONTAP S3 について非対応と記載している 4 項目（Qtree・Quota・FlexClone・ONTAP S3 バケットを含む FlexGroup ボリュームのクローン）は、この経路では制約として現れなかった。実際の落とし穴は別で、NTFS ボリューム + UNIX identity + CIFS サーバ無しの SVM は取り付けが `AVAILABLE` になったうえで全データ操作が `AccessDenied` になる。また ONTAP 側で作ったボリュームが AWS 側の API に現れるまで、ギャップ無しの観測で FlexGroup は 599 秒、FlexClone ボリュームは 1,177 秒かかり、別の回は 1,258 秒でまだ未出現（AWS の記載は「数分」、3 回一致しないので上限ではない）。**測っていないものは未検証として扱う。** 項目別は[相互運用性](reference/limits/s3ap-interoperability.md)（問いの出どころは [ONTAP S3 interoperability](https://docs.netapp.com/us-en/ontap/s3-config/ontap-s3-interoperability-concept.html)） |
| IaC | `AWS::FSx::S3AccessPointAttachment` で宣言できる。update ハンドラが無く `S3AccessPoint` が create-only なので、**ポリシーを変えるだけで削除と再作成になる**。タグは付けられない。ONTAP の識別情報はユーザー名で、数値 UID / GID は OpenZFS 側の項目（[パターン](../../patterns/collect/s3-access-point-attachment/README.md)） |
