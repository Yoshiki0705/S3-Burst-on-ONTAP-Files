...
> **このリポジトリだけでは閉じられない項目である。** 理由と、代役で既に埋めた範囲を先に書く。
>
> | 何が要るか | 状態 |
> |---|---|
> | **AWS の外にある ONTAP クラスタ**（AFF / FAS / ONTAP Select）と、そこへのピアリング経路 | **用意できていない。** AWS のリソースだけでは作れない |
> | 読み手が遠い場合の挙動 | **代役で測定済み。** origin を東京、cache を大阪にして**往復 9.7 ms** で測った（1 ストリーム 602.0 対 101.5 MB/s、8 ストリームでは 1.36 倍まで縮む）。**都市間接続の代役であって、オンプレミスではない** |
> | ピアリングと FlexCache の手順・踏む罠 | **FSx for ONTAP → FSx for ONTAP で通してある。** 経路がオンプレミスかどうかで変わらない部分は流用できる |
> | 継承・削除順序・API の制約 | 同上。踏んだ順に記録してある |
>
> **残っているのは「Cache が AWS の外の ONTAP であること」に固有の部分だけである。**
> 拠点側のバージョン差（9.5 以降で足りるか）、MTU が揃っていない経路での大きい読み取り、
> 拠点のファイアウォールを通したピアリング、**サブミリ秒ではない遅延でのセキュリティスタイル継承**。
>
> **代役で埋まらない理由**: 2 つ目の FSx for ONTAP は AWS の中にあり、ONTAP のバージョンも
> ネットワークも AWS が決める。**拠点側で変わるのはその 2 つ**なので、代役では触れない。
> **ONTAP Select やシミュレータで代替できるかは未確認である**（FSx for ONTAP とのピアリングが
> 成立するかを確かめていない）。
