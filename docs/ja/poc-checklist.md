...
**スループットの合格線には窓の長さを書く。** 第二世代 1,536 MBps の構成で、1 MiB 逐次読み取りが
**27 分間 2,882 MB/s、そのあと 1,439 MB/s** だった（[実測](verification/throughput-capacity-burst-and-baseline.md)）。
**窓を書いていない合格線は 2.0 倍のどこかで成立する。** ワークロードが 30 分未満で、
30 分以上空けて走るならバースト側、連続または間隔が 30 分未満ならベースライン側を線にする。
**判断できないならベースライン側にする**（過大評価しない側）。
...
必要な環境: 項目 1 と同じ。**追加の構築は要らない。**

**この項目は「動くか」ではなく「誰が何をしたかを後から示せるか」を見る。** 監査要件のある環境では
ここが採否の分岐になり、**構成を組んでから聞かれると配布層まで作り直しになる。**

**結論は既に出ている。自分の環境で確かめる価値があるのは、版と設定が違うからである。**

- [ ] **S3 Access Point 経由の操作が FPolicy 通知を発火しないことを確認する。**
      発火しない（実測 2026-08-26 / ONTAP 9.18.1P3D1、UNIX / WINDOWS 識別情報の両方）。
      **`mandatory` 指定の同期ポリシーでも遮断されない。** FPolicy の event が受け付けるのは
      `cifs` / `nfsv3` / `nfsv4` のみで、`s3` は HTTP 400 で拒否される
      （[検証状況](verification-status.md)）
- [ ] **ONTAP ネイティブ監査ログ（`vserver audit`）に載ることを確認する。**
      載る。`Source=HTTP` / `Source=S3` で記録される。**イベント駆動の代替にはならないが、
      監査の跡としては使える**
- [ ] **どちらを監査の根拠にするかを決め、記録する。** FPolicy を前提にした設計を持ち込むと、
      収集層の書き込みだけが監視から抜ける。**抜けていることに気づく経路が無い**のがこの項目の
      危険な点である
- [ ] **イベント駆動が要件なら、ここで判断を止める。** ポーリングか、ネイティブ監査ログを起点に
      するかのどちらかになる（[決定木](reference/decision-trees/choosing-this-architecture.md)の分岐 3）
