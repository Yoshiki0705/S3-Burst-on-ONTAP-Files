...
| `nconnect` マウントオプション | **非対応（ただし拒否ではない）** | **対応**（最大 16 接続） | **実測。両側で接続数を数えている**（下） |

実測は ONTAP 9.18.1P5、SINGLE_AZ_2、Amazon Linux 2023（カーネル 6.18.44）で行った。

> **`nconnect` の可否は、実効オプションではなく接続数で判定した。** 実効オプションに
> `nconnect=16` が現れることは、EFS でも FSx for ONTAP でも起きる。**分かれるのはその先である。**
>
> | | EFS | FSx for ONTAP |
> |---|---|---|
> | マウント | 成功 | 成功 |
> | 実効オプションに `nconnect=16` | 現れる | 現れる |
> | **実際に開いた TCP 接続** | — | **16 本**（`ss` で計数） |
> | I/O | **停滞する**（VDBENCH は 512 スレッド中 495 未完了で `Run aborted`、`dd` も 90 秒でタイムアウト） | 単一接続の 8 倍 |
>
> **FSx for ONTAP 側は v3 / v4.0 / v4.1 / v4.2 のいずれでも 16 本開いた。** AWS の記載は
> 「NFSv3 と NFSv4.1+」で v4.0 を含まないが、v4.0 でも 16 本開き 4,803.59 MB/s 出ている。
> **動いたことはサポートされていることではないので、これを根拠に本番構成にしない。**
> 詳細は[プロトコル別測定の結果](perf-matrix-results.md)。
