...
7. **`tcp-max-xfer-size` が既定 64 KiB**（`rsize=1048576` を要求したマウントが成功したまま
   64 KiB になる。上げると同じ測定が **+21%**）
8. **多重度を 8 のままにする**（16 にすると **+14%** 伸びてファイルシステム側が飽和する。
   **1 台で測った値は上限ではありません**）

> **7 と 8 は同じ測定で分かりました**（2026-09-19/20、第一世代 2048、IOPS 40,000）。
> 1,195.27 → 転送サイズ 1 MiB で 1,446.66 → 16 スレッドで 1,646.17。
> **16 以降は応答だけが倍々に伸びて頭打ちです**（8.208 → 16.420 → 32.838 ms）。
> **8 を直さないまま「クライアント側が限界」と結論すると外します。**
...
make preflight-post FS=… IID=… SECRET=… SMB_SVM=smb_svm    # SMB を測るなら SVM 名を足す
```
...
| **SMB Multichannel**（`SMB_SVM=` を渡したときだけ） | サーバー側が無効なら指摘する。**CIFS サーバーが無ければ「判定できなかった」として止まる** |
| 自動バックアップ・スナップショットポリシー・インライン効率化 | 既定のままなら指摘する |
| `tcp_max_transfer_size` | 判定しない。記録に載せるために表示する |

> **SMB の行は 3 つに分かれ、どれになるかで意味が違います。** 2026-09-19 に実機で 3 つとも
> 通しました（ONTAP 9.18.1P6、128 MBps のクラスタ）。
>
> | 状態 | 出力 | 読み方 |
> |---|---|---|
> | CIFS サーバーが無い | **判定できなかった（止まる）** | **「無効ではない」ではありません。** 読めなかっただけなので、無効のまま測る事故を防ぐために止めます |
> | CIFS サーバーがあり、既定のまま | 指摘（finding） | **有効化してからセッションを張り直して測ります** |
> | 有効化済み | 合格 | それでも負荷中に `CurrentChannels` を読んでください |
>
> **`SMB_SVM=` を渡さなければ SMB の行は出ません。** SMB を測る予定なのに渡し忘れると、
> **無効のまま測って単一チャネルの 574 MB/s を「SMB の性能」として記録します。**
> この引数が Makefile から渡っていない不具合が実際にあり（2026-09-19 に修正）、
> **いまはフラグの受け渡し自体をテストで固定しています。**

> **作ったばかりのスタックでは finding が 3 件出ます。** 自動バックアップ 30 日、
> スナップショットポリシー `default`、インライン効率化 有効 — **テンプレートの既定がそうなっている
> ためで、故障ではありません。** 上の[作成後に 4 つ変えます](#v-1--v-2--v-3--収集側とファイル経路environmentsaws-origin)の
> 表と同じ 3 つです。**測らないなら直す必要はありません。測るなら 3 つとも直してから測ります。**
>
> **128 MBps の構成では NVMe リードキャッシュの行が「該当なし」になります。** そのクラスから
> external-cache のオブジェクト自体が返らないためで、**空の応答をここでは答えとして扱います**
> （2,048 MB/s 以上で空が返った場合は、読み取りが壊れたものとして止まります）。
> **この区別は 2026-09-19 に最小構成で実行して見つけました** — 区別が無かった版は、
> quickstart が作る構成でゲートが止まりました。

**撤去のあとは残留を掃きます。**

```bash
make sweep                          # 報告のみ
make sweep DELETE=1 YES=1           # 実行（最終バックアップ・未アタッチ EBS は取り消せません）
```

**`DELETE=1` だけでは消えません。`YES=1` も必要です。** そして**プロジェクトのものと名前で
結びつかない資源は `--delete` でも消しません** — 判定できないものは、個別に消すコマンドを
出力して残します。**共有アカウントでは、出力の一部は自分のものではありません。**
