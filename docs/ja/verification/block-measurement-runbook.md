...
**900 GiB のボリュームに 600 GiB を 2 つは入らない。** F-1 と F-2 が別の run になり、
別のファイルシステムになったのはこのためである。

> **制約はボリュームのサイズで、構成の不可能性ではなかった。** LUN と namespace を 1 つの
> ボリュームに同居させる構成は AWS が文書に持っており、restricted 状態のボリュームの節が
> 「iSCSI LUN、NVMe/TCP namespace、**または両方**を含む」と書いている
> （[Your volume is in a MISCONFIGURED state](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/misconfigured-volume.html)）。
>
> ブロックボリュームだけサイズを分け（`BLOCK_VOLUME_SIZE_GIB`、既定 1,800 GiB）、
> 600 GiB の LUN と 600 GiB の namespace を同じボリュームに置けるようにした。
> **これで F-1 と F-2 を 1 つのファイルシステムで並べられる。** SSD は 4,096 GiB あり、
> ファイル用 900 GiB と合わせて 2,700 GiB で収まる。
>
> **公開済みの 1,135.18 と 591.64 は別のファイルシステムのままである。** 手順が変わっても
> 過去の数値は変わらないので、同一ファイルシステムでの値は測り直して別に載せる。
...
- **ANA マルチパスを使った NVMe/TCP。** 測定に使ったカーネルは `# CONFIG_NVME_MULTIPATH is not set`
  ...
  **この限定は 2026-09-15 に外れた。** `amazonlinux` リポジトリのカーネルパッケージから config を
  取り出し、**6.1.186 / 6.12.103 / 6.18.48 の 3 系統すべてで未設定**を確認した
  （[F-5](perf-matrix-results.md#ana-マルチパスが-amazon-linux-2023-で使えないこと)）。
  **AL2023 では ANA を使えない。** ANA 有効時の値は別ディストロでしか測れず、未測定
- **ファイルシステムを載せた場合の値。** raw device の値である
- **ベースライン。** 窓は 300 秒でバーストを含む
- **公開済みの F-1 と F-2 の組から読めるプロトコル差。** 2 つは別のファイルシステムで測っており、
  デプロイ間の差が入る。**手順の制約ではなくデータの性質**なので、同一ファイルシステムで
  測り直すまで残る（ボリュームのサイズを分けた経緯は[上](#実行順序)にある）
