...
- **NVMe/TCP との比較はまだできない**（F-2 は[未測定](#未測定)）。
...
| **iSCSI と NVMe/TCP の同一セッション数での比較（F-2）** | **未測定。** F-1 と同じ環境で着手し、**namespace と subsystem は作れたが map と host の追加が失敗した**（private CLI の passthrough は POST を create に写すのに対し、この 2 つは `add` コマンドで、`invalid operation` が返る）。**修正済み・再実行前。** NVMe/TCP 自体は AL2023 で利用可能（`nvme_tcp` と `dm_multipath` がロードされ、`nvme-cli` 2.13）。ただし `/sys/module/nvme_core/parameters/multipath` は**このカーネルに存在しなかった**ので、AWS の手順が `Y` を期待する検証手順はそのままでは成立しない |
| **ブロックでの 2 つの上限（キャッシュ常駐 対 非常駐、F-3）** | 未測定。F-1 と F-2 の速い側で行う設計なので、**F-2 の完了が条件**。ファイル側では 0.18 倍だった |
