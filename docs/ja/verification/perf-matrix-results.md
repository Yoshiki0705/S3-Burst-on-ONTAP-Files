...
> [F-8](#f-8-経路別の内訳と2-つの階層の原因) は逐次読みの値が滞留で 1.85 倍動くことと、
...
**逐次読みの 1.73 倍は滞留の減衰であり、4 KiB ランダム読みの 1.53 倍は未解決である。**
...
> **4 KiB ランダム読みは滞留で動かない。** 温かい側の 131,506.9 IOPS に対し、冷えた側は
> 131,550.6 / 131,749.4 / 131,736.5 で、幅は 0.2% である。**デプロイをまたぐ
> 131k / 203k / 242k のばらつきはこの原因では説明できず、未解決のまま残る。**
...
| 充填をやり直すと 1,239.15 → 2,298.54 | **1.85 倍は滞留で作れる** |

**したがって F-7 の逐次読みの 1.73 倍は、政策の差ではなくセッション内の滞留の減衰である。**
`queue-depth` の一式が先に走り、残りが後に走った順序と一致する。

**4 KiB ランダム読みの 1.53 倍は説明できていない。** 滞留では動かないことを上で測っており、
1 つの実行の中での切り替えは逐次読みでしか試していない。**小さい I/O では政策が効く可能性は
残っており、この測定はそこを閉じていない。**

### この測定が答えていないこと

**下の 4 件は [F-9](#f-9-滞留を落とすもの指定-iops-との比例クライアント側の上限) が答えた。**
残っているのは 4 KiB ランダム読みのデプロイ間のばらつきと、Rocky の冷えた側の書き込みである。

- ~~**温かい側の 2,298 MB/s を決めているもの。**~~ → **クライアント側**（F-9 D）
- **4 KiB ランダム読みがデプロイごとに 131k / 203k / 242k に散る理由。** 滞留では動かず、
  **指定 IOPS でも説明できない**（F-9 B。全デプロイが 200,000 指定で、203,730 はその 102%）
- ~~**4 KiB ランダム読みでの政策の効き。**~~ → **効かない**（F-9 C）
- ~~**滞留が落ちるまでの時間。**~~ → **時間では落ちない。書き込みが落とす**（F-9 A）
- **Rocky の冷えた側の書き込み。** RHEL との書き込みの比較は同状態になっていない
- **`nvmf_lif` が行を返さない理由**

## F-9 滞留を落とすもの、指定 IOPS との比例、クライアント側の上限

**滞留は読みでも経過時間でも落ちない。落とすのは書き込みである。** そして
...
**F-8 の「時間または他の活動で落ちる」はここで確定する** — 落とすのは書き込みである。
...
**これはデプロイ間の 131k / 203k / 242k を説明しない。** F-5 / F-6 / F-7 / F-8 はすべて
SSD 4,096 GiB・プロビジョンド 200,000 で、**指定値は同じである。** しかも
**F-6 の 203,730 IOPS は指定値の 102% にあたる。指定値を超えている以上ディスク律速ではなく、
131k と 203k は別のものに当たっている。** 高い側が何に当たっていたかは未確認である。
...
- **4 KiB ランダム読みがデプロイをまたいで 131k / 203k / 242k に散る理由。** 指定 IOPS では
  説明できず、高い側は指定値を超えているのでディスク律速でもない
- **1 台のクライアントの上限を決めているもの。** クライアント側だと分かっただけである
- **逐次書きでも滞留が落ちるか。** 4 KiB ランダム書きでしか当てていない
- **滞留が落ちるまでに要する書き込み量。** 300 秒の 1 点のみ
...
| **同じ構成の逐次読みが環境をまたぐと 2.64 倍ぶれる理由** | **実測（2026-09-16）。滞留である**（[F-8](#2-つの階層の原因としての滞留)）。同一デプロイで充填し直すと 1,239.15 → 2,298.54 MB/s に戻り、経路・政策・接続順・キュー数・ディストリビューションは潰した。**経路の 1.33 倍という当初の帰属は取り下げ**（経路別に数えると 2 本目は ±0）。以前の記録は下のとおり。 3 環境で 1,288.87 / 2,366.49 / 3,401〜3,408 MB/s。**1 つの接続の中では 0.18% で再現する**ので測定のばらつきではない（[実測](#f-3-と再現性の実測)）。候補は「ANA が使えないため、どちらのコントローラのデバイスを測ったかがデプロイごとに変わる」こと。**次はこれを潰す**（同じ run で両方のデバイスを測り、ノードを ONTAP 側で読む） |
| **ブロックで 2 つの上限に分かれない理由** | **実測（2026-09-16）。分かれる。ただし変数が違った**（[F-8](#2-つの階層の原因としての滞留)）。F-3 が振ったのは**作業セットの大きさ**で、60 GiB と 600 GiB は 3,407.55 対 3,407.59 と一致した。F-8 が振ったのは**書いてからの経過**で、1,239.15 対 2,298.54 に分かれた。**冷えた側の上限は SSD の IOPS**（`DiskIopsUtilization` 最大 95.0〜99.7%）。温かい側は公開されている利用率のどれも上限に無い |
...
| **ブロックでの 2 つの上限（キャッシュ常駐 対 非常駐、F-3）** | **実測（2026-09-13）。作業セットの大きさでは分かれなかった。書いてからの経過では分かれる**（[F-8](#2-つの階層の原因としての滞留)）。（[実測](#f-3-と再現性の実測)）。3,407.55 対 3,407.59 で 0.001% 差。ファイル側の 0.18 倍はブロックでは再現しない |
