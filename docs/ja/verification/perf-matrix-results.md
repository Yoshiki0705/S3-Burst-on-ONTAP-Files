...
| **NVMe/TCP の default と multi で逐次読みが 1.84 倍違う理由** | **未測定。** 交渉されたキュー数は同じ 4 で、300 秒の窓の中ではどちらも安定していた（2,366.49 対 1,288.87）。ランダム 4 KiB はほぼ一致 |
| **ANA マルチパスが有効なカーネルでの NVMe/TCP** | 未測定。AL2023 では `# CONFIG_NVME_MULTIPATH is not set` なので、この AMI では測れない |
| **ブロックでの 2 つの上限（キャッシュ常駐 対 非常駐、F-3）** | 未測定。**速い側は NVMe/TCP の documented な手順（2,366.49 MB/s）と決まった**ので、着手条件は満たしている。ファイル側では 0.18 倍だった |
