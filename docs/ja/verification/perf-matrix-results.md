...
### 測定窓を 300 / 450 / 600 / 900 秒に振った（2026-09-18、別環境で作り直し）

**単一チャネルの 574 MB/s は、測定窓の長さで動かない。** 別の日・別のファイルシステム・別の SVM で
作り直し、1 MiB 逐次書き・512 スレッド・600 GiB 1 ファイルを窓長だけ変えて連続実行した。

| 実行 | 測定窓 | 1 MiB 逐次書き | 応答 |
|---|---|---|---|
| w300a | 300 秒 | 574.07 MB/s | 891.631 ms |
| w300b | 300 秒 | 574.15 MB/s | 891.520 ms |
| w300c | 300 秒 | 574.41 MB/s | 891.090 ms |
| w450 | 450 秒 | 574.47 MB/s | 891.016 ms |
| w600 | 600 秒 | 574.22 MB/s | 891.412 ms |
| w900 | 900 秒 | 574.40 MB/s | 891.127 ms |

**最小 574.07 / 最大 574.47、ばらつき 0.07%。** 300 秒を 3 回反復した側も同じ幅に収まる。
そして上の表の単一チャネル値（1 MiB 逐次書き **574.22 MB/s** / 891.40 ms）と
**0.05% 以内で一致した。** 環境を作り直しても再現する数値である。

ファイルシステム側は全窓で上限から遠い（ネットワーク使用率 5.0%、ディスクスループット 10.7%、
`DiskIopsUtilization` 1.5%）。**574 MB/s の天井はファイルシステムではなくクライアント側の経路にある。**

> **この測定は 300 秒 1,698.42 と 900 秒 1,488.03 の差を説明しない。**
> あの 2 点は **4 チャネル**で測ったもので、今回のセッションは
> **サーバー側の `is_multichannel_enabled` を有効にしないまま**測ったため、
> 単一チャネルの経路を 6 回測っている。**窓長の効果を疑うべき経路は 4 チャネル側で、そこは未測定のまま。**
> 単一チャネル側が 900 秒まで完全に平らだったことから、
> **残る候補は「4 チャネルが測定中に減った」になる**（チャネル数はアイドル時に減ることを上で観測している）。
> **これは仮説であり、測っていない。** 次に測るなら、有効化とセッション再確立の後に
> `CurrentChannels` を**負荷中に窓ごとに読みながら**同じ窓長の並びをもう一度回すこと。

**充填に 63 分かかった。** `SD_format` はスレッド 2・128 KiB で走り **162.73 MB/s** だった。
`formatxfersize` を設定していないパラメータファイルを使ったためで、
**この経路は既に一度直している**（設定すれば 315 MB/s になることを記録済み）。
直した知見が、次に作ったパラメータファイルへ移っていなかった。
...
> **ブロック側の全 run に共通する記録の限界。** ここから下のブロックの数値はすべて
> `iorate=max` の飽和点で、**目標 IOPS を振った曲線もカットオフも取っていない。**
> [上のファイルプロトコル側](#fsx-for-ontap-第二世代nconnect16auto_vdbench目標値を振った系列)が
> 示すとおり、飽和点は到達可能な値より低く、応答時間は 1 桁悪く出る（目標 4,400 で
> 4,406 MB/s・8.86 ms、無制限で 4,204 MB/s・121.79 ms）。**曲線を出している auto_vdbench は
> マウントパスにテストファイルを作る作りなので、raw device / NVMe namespace を駆動できない。**
> ブロック側で同じ形を取るには別の駆動方法が必要で、**未着手である。**
>
> **応答時間を実測として記録したのは [F-10 の 2 系列](#1-台のクライアントの上限が同時実行数でないこと)だけである。**
> ほかは VDBENCH の出力にあった値を保存していない。ただし 512 スレッド固定で `iorate=max` なら
> 応答時間は `512 ÷ IOPS` の恒等式になり、F-10 で導出 216.6 ms 対実測 216.588 ms の一致を
> 確認した。**この式で読み替えた値は導出値であって、実測ではない。**
...
**同状態で一致したのは読みだけである**（0.1% 以内）。**書き込みは 2 つとも Rocky 側が温かい側の
値で、同状態ではない** — 上の表の注記どおりである。4 KiB ランダム書きの 195,439.8 対 194,960.6 は
0.3% だが、**状態が揃っていないので「一致した」と読めない。** 逐次書きの 1,265.08 対 1,308.59
（3.3%）も同じ理由で差として読めない。
**以前ここに「4 KiB ランダム書きは 0.3% 以内で一致する」と書いていたのは誤りで、
表の注記と結論文が食い違っていた**（2026-09-20 に訂正）。
カーネルは z-stream が違う（611.55.1 対 611.5.1）が、`nvme-cli` は同一である。

> **逐次書きは別の環境で同状態で測り直した（[F-11](#f-11-rocky-と-rhel-の逐次書き同一状態同一指定-iops)、2026-09-17）。一致しない。**
> 各 6 回で RHEL の中央値が Rocky の +7.0%、RHEL の最小値が Rocky の最大値を上回り、
> 幅は Rocky 29.6% 対 RHEL 2.1% だった。**したがって「リビルドの値を本体の値として読んでよい」は
> 読みに限る。** 4 KiB ランダム書きは同状態で測り直していないので、**この形状については
> 成立とも不成立とも言えない。**
...
- ~~**Rocky の冷えた側の書き込み。**~~ → **測った（[F-11](#f-11-rocky-と-rhel-の逐次書き同一状態同一指定-iops)、2026-09-17）。同状態・各 6 回で一致しない** — RHEL の中央値が +7.0%、
  RHEL の最小値が Rocky の最大値を上回り、幅は Rocky 29.6% 対 RHEL 2.1%
- **`nvmf_lif` が行を返さない理由**
...
**下の 3 件は [F-10](#f-10-増幅率の対照実験配置を崩す条件スレッド数指定-iops-の反転対照) が
測った。候補は否定された。**

- ~~**増幅率が 0.8 と 1.5 に分かれる理由。** 候補は「4 KiB ランダム書きがそのデバイスで
  一度でも走ったか」~~ → **その候補は否定された**（F-10 P1）。新品の充填直後で既に 0.659 だった
- ~~**増幅が 0.8 のときクライアントが 204k で止まる理由。**~~ → 0.8 の側はこの環境では再現せず、
  **1 台の上限が同時実行数でないことは分かった**（F-10 P4）
- ~~**逐次で充填し直しても増幅率が戻らない理由。**~~ → **増幅率は配置でも書き込み履歴でも
  動かない**（F-10 P1、4 状態で幅 1.3%）
...
## F-10 増幅率の対照実験、配置を崩す条件、スレッド数、指定 IOPS の反転対照

**増幅率の候補は否定された。配置を崩すのは小さいランダム書きだけである。** そして
**1 台のクライアントの上限も NFS 側の 4 KiB ランダム読みの上限も、同時実行数ではない。**
指定 IOPS の比例は**両方向で成立**する。

実測日 2026-09-17、`ap-northeast-1`、ONTAP 9.18.1、第二世代 SINGLE_AZ_2、6,144 MBps、
SSD 4,096 GiB、NVMe リードキャッシュ無効、namespace 600 GiB、c5n.9xlarge の Rocky 9.7 と
RHEL 9.7、NFS 側は AL2023 から `nfsvers=4.1`・`nconnect=16`。VDBENCH 5.04.07、`o_direct`。

> **プロビジョンド SSD IOPS を 100,000 で作成し、途中で 200,000 へ上げた。**
> [F-9 に欠けていた反転対照](#4-kib-ランダム読みが指定-iops-に比例すること)を取るためである。
> 下げる向きは戻す要求が 6 時間のクールダウンで拒否されるので、**上げる向きなら更新 1 回で済む。**

### 増幅率が書き込みの履歴で決まらないこと

同一デバイスで 4 KiB ランダム読みを 4 状態で測った。

| 状態 | 4 KiB ランダム読み | 指定に対して |
|---|---|---|
| **新品の充填直後**（4 KiB ランダム書きが一度も走っていない） | 65,947.6 IOPS | **0.659** |
| 4 KiB ランダム書き 300 秒の後 | 65,430.8 IOPS | 0.654 |
| 逐次で充填し直した後 | 65,070.0 IOPS | 0.651 |
| 200,000 へ上げた後 | 131,529.7 IOPS | **0.658** |

**新品の充填直後で既に 0.659 である。** F-8 / F-9 の 0.657 / 0.660 と同じで、
**[F-9 が挙げた候補](#4-kib-ランダム読みの増幅率はこの配置とは別の変数であること)は否定された。**
100,000 での 3 状態の幅は 1.3%。**配置でも書き込みの履歴でも増幅率は動かない。**

**F-6 / F-7 の 0.83〜0.97 は依然として説明できない。** F-8 以降との違いとして残っているのは
**ONTAP のパッチ版**（F-7 の記録は 9.18.1P6、F-8 以降は 9.18.1）と**測定に使ったクライアント**
だが、**どちらも 1 つの run の中で振っていないので候補として挙げるだけである。**

### 配置を崩すのが小さいランダム書きだけであること

| 段 | 1 MiB 逐次読み | 基準からの差 |
|---|---|---|
| 基準（断片化していない） | 2,364.78 MB/s | — |
| 4 KiB ランダム書きを **30 秒**の後 | 2,343.63 MB/s | **−0.9%（崩れない）** |
| 4 KiB ランダム書きを **300 秒**の後 | **546.02 MB/s** | **−77%（崩れる）** |
| 逐次で充填し直した後 | 2,364.18 MB/s | −0.03%（完全に戻る） |
| **逐次書き 300 秒**の後 | 2,365.59 MB/s | **+0.03%（崩れない）** |

- **30 秒では崩れない。300 秒で崩れる。** 閾値はこの間にある
- **逐次書きでは崩れない。** [F-9 の「4 KiB ランダム書きが崩す」という狭い書き方が正しかった](#配置を崩すのが書き込みであること)。
  **「書き込みが崩す」と広げてはいけない**
- **崩れていない側は指定 IOPS に依存しない。** 100,000 で 2,364.78、200,000 で 2,366.23 である
- **崩れた側については、比例を主張できない。** ここでの 546.02（指定 100,000）と F-8 / F-9 での
  1,272.8（指定 200,000）は比 2.33 で、**指定 IOPS の向きには動くが 2.00 ではなく、しかも
  別デプロイの値である。** 崩れた状態で指定 IOPS を振った run が無いので、
  **これは対照実験ではない。** 崩れた側がディスク IOPS で飽和していること
  （[F-9 で 89〜95%](#配置が崩れるとディスク側の-1-回が-75-分の-1-になること)）から
  比例を予想はできるが、**測っていない**

### 1 台のクライアントの上限が同時実行数でないこと

断片化していない状態の 1 MiB 逐次読み。
...
**スレッドを 4 倍にしてスループットは 0.07% しか動かない。** 応答時間だけが比例する。
**未完了要求を増やしても 1 台の上限は上がらない。** 内訳は測っていない。

### NFS 側の 4 KiB ランダム読みも同時実行数でないこと

| スレッド数 | IOPS | 応答時間 | クライアント CPU |
|---|---|---|---|
| 512 | 94,949.6 | 5.391 ms | 27.4% |
| 1,024 | 97,915.3 | 10.456 ms | 38.5% |
| 2,048 | 97,014.5 | 21.108 ms | 48.2% |

**スレッドを 4 倍にして 2.2% しか動かない。**
**[指定値・バースト・ベースラインの実測](throughput-capacity-burst-and-baseline.md)が
分離できないとした 2 択のうち、同時実行数の側が落ちた。** あの節は分離方法として
「スレッド数を振る」か「指定 IOPS を振る」を挙げていた。**前者をここで実施した。**
一致していた `512 / 5.35 ms = 95,700` は Little の法則の恒等式であって、原因ではない。

**94,949.6 は元の測定の 95,630 と 0.7% 差である。** 元は SSD 8,192 GiB、こちらは 4,096 GiB で、
**容量が違っても再現した。** 指定 200,000 の 47.5% で、以前から記録していた
[「NFS 16 接続で 45%」](#この測定でいちばん効いた要因)と同じ範囲にある。
~~**増幅率 約 2.1 に相当するが、この run の増幅率そのものは未確認である**~~
（CloudWatch のデータ点が実行直後には出ず、撤去を優先した。保持は 63 日）。
**その 63 日の中で測った。下の節にある。**

### 4 KiB ランダム読みの増幅率とディスク IOPS の実測（2026-09-20）

**追加のリソースは使っていない。** ファイルシステムは撤去済みだが、CloudWatch は削除後も 63 日
保持する。`DiskReadOperations ÷ DataReadOperations` を 5 分粒度で読んだ。

| 窓（JST） | スレッド | フロント IOPS | ディスク IOPS | **増幅率** | 指定 200,000 に対するディスク側 |
|---|---|---|---|---|---|
| 2026-09-17 01:05 | 512 | 94,322 | 152,241 | **1.61** | **76%** |
| 2026-09-17 01:10 | 1,024 | 98,178 | 150,194 | **1.53** | 75% |
| 2026-09-17 01:15 | 2,048 | 95,835 | 147,054 | **1.53** | 74% |
| 2026-09-17 00:20（対照・ブロック経路） | — | 131,246 | 198,794 | 1.51 | **99.4%** |

**フロント IOPS は F-10 が記録した 94,949.6 / 97,915.3 / 97,014.5 と 0.7〜1.1% で一致する**ので、
同じ run を見ている。

**3 つ読める。**

1. **増幅率は 1.53〜1.61 で、推定していた約 2.1 ではない。** `DiskIopsUtilization` 82% から
   逆算した 1.7 も高すぎた。**利用率から増幅率を逆算すると外れる。**
2. **したがって「残る候補は増幅率」も外れた。** ディスク側は指定値の 74〜76% で、
   **飽和していない。** 同時実行数（F-10 のスレッド掃引で 2.2%）と指定 IOPS の両方が消えた。
3. **対照が効いている。** 同じファイルシステムのブロック経路は同じ増幅率（1.51）で
   **ディスク IOPS を 99.4% まで使い切った。** つまり指定値には到達できる。
   **NFS の 4 KiB ランダム読みだけが 74〜76% で止まる。**

**何が止めているかは依然として未確認である。** ただし候補は 3 つ消えた。

#### 指定 IOPS を 1/4 にしたときの比の不成立（2026-09-20）

**上の 74〜76% は NFS の 4 KiB ランダム読みの性質ではありませんでした。** 世代を第二世代に
固定したまま 6,144 MBps / 200,000 IOPS から **1,536 MBps / 50,000 IOPS** に落として測り直すと、
**ディスク側は指定値の 99.1% に達します。**

| 構成 | スレッド | フロント IOPS | 指定に対して | ディスク IOPS | 指定に対して | 増幅率 |
|---|---|---|---|---|---|---|
| 6,144 / 200,000 | 512 | 94,322 | 47.2% | 152,241 | 76% | 1.61 |
| **1,536 / 50,000** | 512 | **31,175** | **62.3%** | **49,573** | **99.1%** | **1.59** |
| **1,536 / 50,000** | 1,024 | **31,311** | **62.6%** | **49,549** | **99.1%** | **1.58** |

**増幅率は 1.58〜1.59 で、大きい構成の 1.53〜1.61 と同じ範囲です。** 変わったのはディスク側の
充足度だけで、**76% から 99.1% に上がりました。**

**3 つ読めます。**

1. **「指定の 47.5%」という比は再現しません。** 1/4 の指定 IOPS では 62.3〜62.6% です。
   **47.5% はあの構成に固有の値で、規模に対する不変量ではありません。** 引用するときは
   6,144 MBps / 200,000 IOPS という条件を必ず併記してください。
2. **小さい構成ではディスク IOPS が律速です。** フロント側の上限は
   `指定 IOPS ÷ 増幅率` にほぼ一致します（50,000 ÷ 1.58 = 31,646 に対し実測 31,175〜31,311）。
   **ただしこの式が成り立つのはディスク側が飽和しているあいだだけで、大きい構成では成り立ちません**
   （200,000 ÷ 1.61 = 124,224 に対し実測 94,322）。
3. **したがって大きい構成の律速は、小さくすると消える性質のものです。** ディスク IOPS でも
   増幅率でも同時実行数でもない何かが 6,144 / 200,000 側だけに効いています。**依然として未確認です。**

クライアント側の測定器も同じ値を見ています。vdbench は 31,433.7（512 スレッド）と
31,371.9（1,024 スレッド）で、上表のフロント IOPS と 0.2〜0.8% 差です。
**スレッドを倍にしても IOPS は動かず、応答だけが 16.287 → 32.638 ms と倍になりました** —
`512 ÷ 0.016287 = 31,436`、`1,024 ÷ 0.032638 = 31,375` という Little の法則の恒等式で、
**クライアントは待っているだけ、天井はサーバー側**であることの確認です。

測定条件: ONTAP 9.18.1P6、ボリュームは圧縮・重複排除・スナップショットすべてオフ、
200 GiB の非圧縮データセット（`gzip -1` で元の 100.0%）、NFS v4.1 `rsize=65536` `nconnect=16`。
**この階級では `system node external-cache` のレコードが 0 件で、NVMe 読み取りキャッシュは
公開されていません。**

### 指定 IOPS の比例が両方向で成立すること

| 指定 IOPS | 4 KiB ランダム読み | 指定に対して |
|---|---|---|
| 100,000 | 65,070〜65,947 | 0.651〜0.659 |
| **200,000（上げた後）** | **131,529.7** | **0.658** |

**比 2.02。上げる向きでも比例する。**
[F-9 の下げる向きの結果は更新操作の副作用ではない。](#4-kib-ランダム読みが指定-iops-に比例すること)
131,529.7 は F-8 の 131,413.5、F-9 の 131,617 と 0.1% 以内で、**3 デプロイで再現している。**
更新は `UPDATED_OPTIMIZING` に約 18 分留まった（F-9 は約 17 分）。

### Rocky の断片化した状態での書き込み

| ワークロード | Rocky 9.7（指定 100,000） | RHEL 9.7（F-9、指定 200,000） |
|---|---|---|
| 1 MiB 逐次書き | 1,257.34 MB/s | 1,265.08 MB/s（**0.6% 差**） |
| 4 KiB ランダム書き | 188,110.8 IOPS | 195,439.8 IOPS（3.7% 差） |

**状態は揃ったが指定 IOPS が揃っていない。** 逐次書きが 0.6% で一致することから
**逐次書きは指定 IOPS に依存しない**と読めるが、**4 KiB ランダム書きの 3.7% 差を
ディストリビューションの差として読んではいけない。** なおこの run の 4 KiB ランダム書きは
188,110〜202,727 IOPS で**指定 100,000 を超えており、書き込みは指定 SSD IOPS の天井に
当たっていない。**

### この測定が答えていないこと

- **F-6 / F-7 の増幅率 0.83〜0.97 の原因。** 書き込みの履歴では説明できないことが確定した。
  残る違いは ONTAP のパッチ版とクライアントで、**どちらも 1 run 内で振っていない**
- **1 台のクライアントの上限を決めているもの。** 同時実行数でないことが確定した
- **NFS 側の 4 KiB ランダム読みの上限を決めているもの。** 同時実行数でないことが確定した。
  増幅率 約 2.1 が候補だが、この run の増幅率は未確認
- **配置が崩れる閾値。** 30 秒と 300 秒の間である
- **負荷に対する応答時間の曲線。** ブロック側は全 run が `iorate=max` の 1 点で、
  **目標 IOPS を振った曲線もカットオフも取っていない**（[理由](#測定条件上の共通条件からの差分)）。
  応答時間を実測したのはこの run のスレッド振り 2 系列だけである
- **`nvmf_lif` が行を返さない理由**
...
| SMB Multichannel のチャネル数を 4 より増やす条件 | **未測定。ただし 4 の出どころが分かった（2026-09-20）。** クライアント側の `ConnectionCountPerRssNetworkInterface` が 4 で、RSS 対応 NIC 1 枚あたりのチャネル数はこれで決まる。サーバー側の `max_connections_per_session=32` はセッションあたりの上限で、**増やす側の律速ではない。** NIC を増やすかこの値を上げる試験は未実施 |
...
| SMB の 300 秒と 900 秒で値が違う理由 | **仮説を 1 つ否定した（2026-09-20）。値そのものは未解明。** 残っていた「測定中にチャネル数が減った」を直接測り、900 秒の窓で 30 秒ごとに 31 回読んで**全点 4**、待機中も 4 だった（[実測](throughput-iops-concurrency.md#900-秒のあいだチャネル数が減らないこと2026-09-20)）。**測定は第二世代 1,536 MBps なので 1,698.42 / 1,488.03 の大きさは再現していない。** |
...
## F-11 Rocky と RHEL の逐次書き（同一状態・同一指定 IOPS）

[F-8 の一致表](#rhel-本体と-rocky-の一致)は、逐次書きだけ**同状態で測れていない**まま残していた
（RHEL は冷、Rocky は温）。**そこだけを埋めた。結論は「その精度では一致しない」である。**

実測日 2026-09-17、`ap-northeast-1`。**第二世代 SINGLE_AZ_2、1,536 MBps 指定、1 HA ペア、
SSD 1,024 GiB、プロビジョンド 50,000 IOPS**、ブロックボリューム 700 GiB、NVMe/TCP namespace
300 GiB を 1 回充填（664.56 MB/s）。VDBENCH 5.04.07、`o_direct`、`xfersize=1024k`、
`seekpct=.0`、`rdpct=0`、`threads=8`、`warmup=30`、`elapsed=120`、`iorate=max`。

> **F-8 / F-9 / F-10 は 6,144 MBps・200,000 IOPS なので、絶対値をあちらと並べられない。**
> ここで読めるのは、同一ファイルシステム・同一 namespace・同一状態での **Rocky 対 RHEL の差だけ**
> である。**プロビジョンド IOPS を 50,000 にしたのは、既定の 3,072 だと両クライアントが同じ
> IOPS 天井に張り付いて、測りたい差が隠れるためである。**

両クライアントで揃っているもの: c5n.9xlarge、ANA 有効（`multipath: Y`）、`nvme-cli 2.16-1.el9`、
`iopolicy=queue-depth`、交渉されたキュー数 `0x00010001`、同一 namespace UUID、同一デバイス。
**記録されている差はカーネルの z-stream だけ**（Rocky 611.5.1、RHEL 611.55.1）。

| クライアント | n | 最小 | 中央値 | 平均 | 最大 | 幅 |
|---|---|---|---|---|---|---|
| Rocky 9.7 | 6 | 842.70 | 1,040.26 | 1,006.01 | 1,092.50 | **29.6%** |
| RHEL 9.7 | 6 | 1,105.75 | **1,112.65** | 1,113.61 | 1,129.30 | **2.1%** |

単位 MB/s。実行順は A B A2 B2 A3 B3 A4 A5 A6 B4 B5 B6 で、**交互から始めて順序とクライアントを
分離した。**

**確定したこと 2 つ。**

1. **RHEL の中央値が Rocky の +7.0%、そして RHEL の最小値 1,105.75 が Rocky の最大値 1,092.50 を
   上回る。** 6 対 6 で範囲が重ならない。
2. **幅が桁で違う。** Rocky は 29.6%、RHEL は 2.1%。**中央値の差より、この非対称のほうが大きい。**

**したがって「Rocky の値を RHEL の値として読んでよい」は、逐次書きには広げられない。**
読み（0.1% 以内）と 4 KiB ランダム書き（0.3% 以内）で成立した一致は、
**この形状では成立しない。**

**原因は書けない。** 記録されている差は z-stream だけだが、単独で振っていない。
測定中の CloudWatch はディスク TP 使用率 72〜81%、IOPS 使用率 16〜19%、ネットワーク 31〜38% で、
**どれも飽和していない** — ファイルシステム側の天井で両者が揃っていたのではない。

**この節の数値は ONTAP の版を伴っていない。** `FileSystemTypeVersion` は `null` を返し、
クラスタ API から取り直さないまま撤去した。**セッション 1 と同じ抜けで、2 回続けたので
`runbook.sh preflight` に版の読み取りを足した**（読めなければ測定に進まない）。
