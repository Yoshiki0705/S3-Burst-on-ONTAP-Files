...
| **iSCSI のセッション数とスループット（1 対 8）** | **未測定。計画あり**（[ブロックプロトコルの測定計画](block-protocol-matrix-plan.md)）。AWS は 1 クライアント 5 Gbps（約 625 MBps）を documented として書いているが、**この環境のファイル側の実測はその 94.7%（NFS 591.62）と 91.9%（SMB 574.24）だった。** ブロックで同じずれが出るかは未測定 |
| **iSCSI と NVMe/TCP の同一セッション数での比較** | 未測定。計画あり。**両者は SVM の同じ iSCSI LIF を使うが、multipath の分散が dm-multipath と ANA で違う** |
| **ブロックでの 2 つの上限（キャッシュ常駐 対 非常駐）** | 未測定。計画あり。ファイル側では 0.18 倍だった |
