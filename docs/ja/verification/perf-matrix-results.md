...
| 測定 | Elastic | Provisioned |
|---|---|---|
| マウントヘルパー、1 MiB 逐次読み | 1,495.82 MB/s | **498.61 MB/s** |

**Elastic で 3 倍だったヘルパーの効果が、Provisioned では出なかった。** ただし
**この 2 回はスループットモードだけでなくテストファイルの大きさも違う**（300 GiB 対 32 GiB。
Provisioned 側を小さくしたのは、$0.07/GB の書き込み課金と作成時間のため）。**原因は切り分けて
いない。ヘルパーが一律に効くという読みは、この 1 点で取り下げる。**
...
| ケース | 状態 |
|---|---|
| キャッシュ状態をそろえた再測定 | **未測定。上の 45% の幅を畳むために最も価値がある** |
| ヘルパーの 3 倍が Provisioned で出なかった理由（ファイルサイズか、モードか） | **未測定。切り分けには Elastic 側を 32 GiB で測り直す** |
