...
**1 → 2 では逐次スループットが 1 バイトも動かない**（1,135.18 が 2 回）。**上限は 1 フローの
容量ではない。** 16 本にすると読みは 1.74 倍、書きは 2.17 倍、4 KiB ランダムは読み 3.0 倍・
...
> **AL2023 では ANA が使えない。** `/sys/module/nvme_core/parameters/multipath` は存在せず、
> カーネル設定は **`# CONFIG_NVME_MULTIPATH is not set`**、`modinfo nvme_core` にも multipath の
> パラメータが無い。**したがって AWS の手順の「`Y` が返れば成功」という検証はこの AMI では成立せず、**
...
| 比較 | iSCSI | NVMe/TCP | 比 |
|---|---|---|---|
| **1 接続**（iSCSI: 1 セッション / NVMe: I/O キュー 1） | 1,135.18 MB/s | **591.64 MB/s** | **iSCSI が 1.92 倍** |
| documented な手順のまま（iSCSI: 両ポータル / NVMe: `connect-all` 1 回） | 1,135.18 MB/s | **2,366.49 MB/s** | **NVMe が 2.08 倍** |
| その手順で得られる 4 KiB ランダム読み | 44,593 IOPS | **126,964 IOPS** | **NVMe が 2.85 倍** |
| iSCSI の 16 セッション 対 NVMe の `connect-all` 1 回 | 1,970.43 MB/s | **2,366.49 MB/s** | NVMe が 1.20 倍 |
...
| **iSCSI のセッション数とスループット（1 対 8）** | **実測（F-1、2026-09-12）。**[F-1 iSCSI の実測](#f-1-iscsi-の実測)にある。数えた TCP が 1 本で 1,135.18 MB/s、2 本でも同じ値、16 本で 1,970.43 MB/s。**625 MBps の除数は 1 本で 1.82 倍、16 本で 0.20 倍に外れた** |
| **iSCSI と NVMe/TCP の比較（F-2）** | **実測（2026-09-13）。**[F-2 NVMe/TCP の実測](#f-2-nvmetcp-の実測)にある。**「1 接続」では iSCSI が 1.92 倍、documented な手順のままでは NVMe/TCP が 2.08 倍。順序が入れ替わる。** ANA マルチパスを使った値は**未測定**（AL2023 のカーネルは `CONFIG_NVME_MULTIPATH` が無効） |
| **同じ構成の逐次読みが環境をまたぐと 2.64 倍ぶれる理由** | **未測定。** 3 環境で 1,288.87 / 2,366.49 / 3,401〜3,408 MB/s。**1 つの接続の中では 0.18% で再現する**ので測定のばらつきではない（[実測](#f-3-と再現性の実測)）。候補は「ANA が使えないため、どちらのコントローラのデバイスを測ったかがデプロイごとに変わる」こと。**次はこれを潰す**（同じ run で両方のデバイスを測り、ノードを ONTAP 側で読む） |
| **ブロックで 2 つの上限に分かれない理由** | **未測定。** 作業セット 60 GiB（キャッシュの 0.23 倍）と 600 GiB（2.3 倍）で 3,407.55 対 3,407.59 と一致した。**「常駐側もキャッシュから返っていない」と「上限が経路の手前にある」を区別していない** |
| **ANA マルチパスが有効なカーネルでの NVMe/TCP** | 未測定。AL2023 では `# CONFIG_NVME_MULTIPATH is not set` なので、この AMI では測れない |
| **ブロックでの 2 つの上限（キャッシュ常駐 対 非常駐、F-3）** | **実測（2026-09-13）。分かれなかった**（[実測](#f-3-と再現性の実測)）。3,407.55 対 3,407.59 で 0.001% 差。ファイル側の 0.18 倍はブロックでは再現しない |
