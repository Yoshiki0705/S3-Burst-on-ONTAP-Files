...
on the other face, and which figure applies depends on the direction of the writes. **A's figures
were measured on this architecture; B's come from AWS documentation.** They are different kinds of
claim, so they are not put in one table.
...
- **Have the consumers write back and B's roughly 60 seconds becomes the freshness everything
  ...
  appending continues.
- A's counterpart caveat is the mount. On a default mount the client's cache expiry dominates over
  the server-side propagation, and the figure above is with `actimeo=0`.

B's specifications — the synchronisation rate ceilings, the mandatory versioning and what happens on
a conflict — are in
[specifications that decide whether S3 Files fits](reference/comparison/finops-s3-vs-s3ap.md#specifications-that-decide-whether-s3-files-fits),
and A's measurement conditions are in [verification status](verification-status.md).
