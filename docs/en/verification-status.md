...
| Whether an ONTAP S3 NAS bucket (FlexCache duality — separate mechanisms from the S3 Access Point) can be used on FSx for ONTAP | **regular volume: works / FlexCache: no data access** | [All-directions comparison](verification/cross-protocol-directions.md). On a regular volume, NFS write to ONTAP S3 `GetObject` succeeded (the S3 user can be created through the CLI). On a FlexCache volume, creating the NAS bucket and `HeadBucket` succeed but `GetObject` and `ListObjects` return AccessDenied. ONTAP 9.18.1P3D1, FSx for ONTAP |
