...
| Whether Cloud Volumes ONTAP / ONTAP Select / Azure NetApp Files / Google Cloud NetApp Volumes can be the cache when FSx for ONTAP is the origin | **unconfirmed, but each cloud's requirements are now known (researched 2026-09-20)** | **AWS's [supported configurations](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/using-flexcache.html) list exactly three** (on-premises ONTAP as origin with FSx for ONTAP as cache, the reverse, and FSx for ONTAP to FSx for ONTAP). **The other side's documentation states requirements for an "external ONTAP origin" without naming platforms.** ANF: [cache requirements](https://learn.microsoft.com/en-us/azure/azure-netapp-files/cache-requirements) demand **ONTAP 9.15.1 or later on the source cluster**, encryption on the origin volume, cluster peering, Standard network features, REST only. GCNV: [creating a FlexCache](https://cloud.google.com/netapp/volumes/docs/configure-and-use/volumes/cache-ontap-volumes/create-flexcache-volume) demands a **FlexVol or FlexGroup on an ONTAP cluster**, **access-time updates disabled on the origin**, and peering initiated from GCNV with **`cluster peer create` and `vserver peer create` run on the origin cluster**. **FSx for ONTAP can meet both sets, but neither side names the combination.** For CVO an AWS [blog](https://aws.amazon.com/blogs/storage/caching-data-using-amazon-fsx-for-netapp-ontap/) says it can be an origin, which **is not service documentation**. **ONTAP Select is named by none of the three** |
| **The reverse** — GCNV or ANF as origin with FSx for ONTAP as cache | **GCNV is documented, conditionally (2026-09-20). ANF is unconfirmed** | **GCNV: [ONTAP-mode volumes can be an origin or a cache; all other volumes can only be caches](https://cloud.google.com/netapp/volumes/docs/configure-and-use/volumes/cache-ontap-volumes/create-flexcache-volume).** So GCNV can be an origin only in ONTAP mode, and **cannot** at the ordinary service levels. **ANF documents itself as the cache and states nothing about being an origin.** Neither appears in AWS's table. Being ONTAP-based is not the basis for either ([portability](portability.md)) |
| Cluster peering over a managed service (AWS Interconnect – multicloud), and a FlexCache read over that path | unconfirmed | The service is documented — GA for Google Cloud and OCI, Preview for Azure since 2026-08. **No Japanese pair exists, so following it on hardware means building an environment in a Region that is paired** ([cross-cloud connectivity](multi-cloud-connectivity.md)) |
| Whether AWS Interconnect – multicloud's physical link encryption is MACsec | unconfirmed | The product page names no standard. MACsec is named only by an AWS executive quoted on a Microsoft blog, and is **not corroborated in the service documentation** ([cross-cloud connectivity](multi-cloud-connectivity.md)) |
...
| C | Re-measure the FlexCache side incompressibly | **Done.** Resident 882.7 to 907.1 against a direct origin read of 383.3, so **FlexCache is 2.31 times faster.** "53% of the origin" had it backwards. **Raising only the origin to 2048 MBps on 2026-09-18 turns it around again: the origin is 8.1 times faster.** The ratio is set by the pair of throughput steps, not by FlexCache alone |
...
|---|---|---|
| ~~Read at least twice the cache in one pass~~ | **Measured (2026-09-17).** An 800 GiB volume and a 512 GiB file, 2.15x this step's 238 GiB cache: `AUTOMATIC` 3,072 read 256.47 MB/s and 40,000 read 1,246.67 MB/s (**4.86x**). **The 7.14x does not reproduce.** The low point saturates at `DiskIopsUtilization` 112% with 82 to 103% of the bytes coming from disk. **The high point has no established ceiling** (disk throughput 67%, IOPS 43%, network 43%, and going from 8 to 64 threads moves it from 1,071 down to 958). [The re-measurement](../ja/verification/throughput-iops-concurrency.md#キャッシュを超える作業セットでの再測定2026-09-17) (Japanese) | Done |
| ~~What holds the high point at about 1,250 MB/s~~ | **Identified (2026-09-19/20): not a resource, but two client-side settings.** Transfer size (`rsize` 64 KiB to 1 MiB moves 1,195.27 to **1,446.66**, +21%) and concurrency (8 to 16 threads reaches **1,646.17**, then flat to 64). **From 16 threads on, `FileServerDiskThroughputUtilization` pins at 102.6%** (77.2% at eight) while **only the response time doubles** with each doubling (8.208, 16.420, 32.838 ms). **IOPS at 50.9% and network at 66.3% are not saturated at any point.** So what saturates is **the file system side**, and one client reaches the same 102.6% that two clients reached. **"The ceiling is on the client side" (from the two-client run of 2026-09-19) holds only for the configuration measured there; it was not a per-client resource limit.** No saturated client-side device can be named, because there was not one. [The measurement](../ja/verification/throughput-iops-concurrency.md#スレッド数を振った結果--1195-を決めていたのは設定-2-つ2026-09-1920) (Japanese) | Done |
| ~~What holds the 4 KiB front end at 6,144 MBps / 200,000 IOPS~~ | **Identified (2026-09-20): it is the client.** Changing only how the same working set is split, on one file system: **more volumes gives +1.0%, a second SVM +26.3%, a second client +52.7%**. **At two clients the disk side reaches 96.6% of the provisioned 200,000** -- the state one client already produced at 50,000 provisioned IOPS (99.1%). **So the server was never the limit at 200,000; one client was.** The lever is not threads (512 to 1,024 gives +8.9%) but **how many distinct server IPs the client talks to, and how many clients there are**. Amplification is 1.54-1.67 across all five shapes and does not move. **Not a reproduction of F-10's 94,949.6** (baseline 81,313.9, 14% lower; F-10's working set was never recorded, so like-for-like is not available). [Measurement](../ja/verification/throughput-iops-concurrency.md#4-kib-ランダム読みの天井の所在2026-09-20) (Japanese) | Done |
| ~~The 15% gap between about 1,650 (one client) and about 1,901 (two)~~ | **Closed (2026-09-20, no new resources). The premise was wrong.** **This configuration publishes no metric containing `Balance` at all** (0 of 77 on both file systems), so there was no burst allowance to equalise. And `DiskReadBytes / (utilization / 100)` converges on **2,048 MB/s, the specified throughput capacity**, at every minute — on the first generation the utilization figure is a ratio against the **specified value**, so above 100% means it delivered more than that, not that a credit was consumed. **Both sessions pinned server-side `DataReadBytes` at about 2,040 MB/s**, the same ceiling, with read amplification at 102 to 106% in both. **The 15% difference in the client-reported totals is not a difference in what the file system delivered.** [Detail](../ja/verification/throughput-iops-concurrency.md#保持された-cloudwatch-で閉じた-2-点2026-09-20) (Japanese) | Done ($0) |
| ~~Server-side counters disagree with the client-side tool~~ | **Closed (2026-09-20). It was a measurement-method finding, not a product discrepancy.** Start the window on a minute boundary and compare only the whole minutes wholly inside it, and **the two agree to within 0.2% in both directions** (300 s window: vdbench 271.04 against CloudWatch 271.57, +0.20%; 900 s window: 326.70 against 326.07, -0.19%). **The earlier disagreement came from taking, as the server-side figure, minutes that carried load for only part of their 60 seconds** -- visible in the very minutes this run excludes (216.85 at the start and 58.47 at the end, against a steady 271). **Nothing about FSx for ONTAP needs restating.** [Measurement](../ja/verification/throughput-iops-concurrency.md#窓を分境界に合わせた-2-系統の比較2026-09-20) (Japanese) | Done |
| ~~The SMB gap between a 300-second and a 900-second window (1,698.42 against 1,488.03)~~ | **Closed (2026-09-20). It does not reproduce.** Re-measured at 6,144 MBps, the long window is **4.0% HIGHER** (short 1,375.4 against long 1,430.3 MB/s). The original had the long window 12.4% lower, so **the direction is opposite**. The channel count was 4 at all 14 readings (a second confirmation after 31 at 1,536 MBps), and varying only the duration moves a write by 1.9%. **Refuted from three directions, so the original pair reads as a one-off rather than a property of the window length.** **Limitations**: the long window aborted at about 480 s when vdbench's format phase hit `data_errors=50`, so it is a 300-against-480 comparison; and the absolute level is not comparable because the shape differs (16 files of 40 GiB against a single-file overwrite). [Measurement](../ja/verification/throughput-iops-concurrency.md#smb-の窓長-124-の再現不成立2026-09-20) (Japanese) | Done |
| ~~Vary only the run duration~~ | **Measured (2026-09-20). The duration is not the cause.** On second-generation 1,536 MBps with 50,000 provisioned IOPS, each duration run twice and interleaved, **writes came to 1,270.2 / 1,273.3 / 1,294.4 MB/s at 30 / 60 / 90 s, a spread of 1.9%** -- and **two runs at the same duration differ by more than that** (4.4% at 30 s). **The 16% is carried by what else differed: 3,072 against 40,000 provisioned IOPS, payload compressibility, and the file system instance.** Reads put 30 / 90 / 300 s within 1.4% of each other and **60 s alone 8.6% higher**, reproduced in both passes and not monotonic in duration. **Limitation**: the original pair was a first-generation 2048 MBps write, so the magnitudes do not transfer. [Measurement](../ja/verification/throughput-iops-concurrency.md#測定時間だけを振った結果--16-の不在2026-09-20) (Japanese) | Done |
| ~~FlexCache incompressibly at the 2048 MBps step~~ | **Measured (2026-09-18), and it inverts.** With the origin at 2048 and the cache at 128, resident read 210.59 against a direct origin read of 1,706.67 MB/s -- **the origin is 8.1 times faster**. The first-fill penalty also falls from 2.88x to **+1.5%**, because both paths stop at the cache's own ceiling. **The resident figure is a burst figure** (cache-side disk 190 to 214%, network 325 to 365%) and **the direct origin read is a warm control** (origin-side `DiskReadBytes` is 0). ONTAP 9.18.1P6 on both clusters. [Details](../ja/verification/throughput-iops-concurrency.md#origin-を-2048-mbps-に上げたときの向きの逆転2026-09-18) (Japanese) | Done |
