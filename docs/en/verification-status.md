...
| The 15% gap between about 1,650 (one client) and about 1,901 (two) | **Unconfirmed.** Both pin `FileServerDiskThroughputUtilization` at 102.6% yet differ in absolute terms. **102.6% is a ratio against a baseline while bursting, and burst allowance was not equalised between the two sessions.** Which published figure the absolute value corresponds to is also unreconciled | One file system and two clients, about an hour. **Read the burst allowance first and equalise it** |
