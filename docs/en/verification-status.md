...
| **Server-side counters disagree with the client-side tool** | **Unconfirmed — opened, more sharply, by closing the row above.** At the saturated minutes the server reports about 2,043 MB/s served while vdbench reports 1,646 to 1,649, a 19% shortfall; with two clients it is about 2,000 against 1,901, a 5% shortfall. Candidates not separated: vdbench's run average spans its own ramp-in, the byte counters may include replies the application never counted, MB versus MiB in one tool. **Two systems that disagree are not used as a ceiling, so neither figure is promoted** | **No new file system.** One run whose vdbench window and CloudWatch window are aligned on purpose |
| The SMB gap between a 300-second and a 900-second window (1,698.42 against 1,488.03) | **Measured, and it does not answer the question (2026-09-18).** The ladder -- 300 seconds three times, then 450, 600 and 900 -- was flat to **0.07%**, but **the session ran with server-side Multichannel left disabled, so it measured the single-channel path six times.** That path's 574 MB/s reproduced to within 0.05% on a rebuilt environment. **The four-channel path, which is where the two numbers came from, is unmeasured.** The remaining candidate is that the channel count fell during the run, which is a hypothesis | One file system and a Windows client, about two hours. **Read `CurrentChannels` per window while under load** |
