...
| The SMB gap between a 300-second and a 900-second window (1,698.42 against 1,488.03) | **The remaining hypothesis is refuted (2026-09-20), though the magnitudes are still unexplained.** "The channel count fell during the run" was measured directly: `CurrentChannels` read every 30 s across a 900 s window, 31 samples, **4 at every one of them**. It was also 4 before the load and 10 s after it stopped, so it did not even fall while idle -- the hint in `template-windows.yaml`'s output that it drops when idle did not hold here. **The channel count is not the cause.** **Stated limitation**: this ran on the second generation at 1,536 MBps, so the 1,698.42 / 1,488.03 **magnitudes are not reproducible here and reproduction was not attempted**. Only the channel-count hypothesis was tested. [Measurement](../ja/verification/throughput-iops-concurrency.md#900-秒のあいだチャネル数が減らないこと2026-09-20) (Japanese) | Done, apart from explaining the magnitudes |
