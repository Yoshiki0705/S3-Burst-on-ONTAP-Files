...
| What holds the high point at about 1,250 MB/s | Left open by that re-measurement. The candidate is the SVM's `tcp_max_xfer_size`, measured at 65536. **Raising it to 1 MiB also moves where the low point saturates on IOPS, so both points have to be re-measured in one session.** Lowering provisioned IOPS carries a six-hour cooldown | One file system, about four hours |
