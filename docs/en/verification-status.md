...
| ~~What holds the high point at about 1,250 MB/s~~ | **Identified (2026-09-19): the client side.** Two clients reading disjoint 300 GiB regions of one volume total **1,901.48 MB/s against 1,195.27 for one -- 1.59x**. **With one client disk throughput sits at 63% and IOPS at 40%, neither saturated; with two, disk throughput reaches 102 to 103%** (above baseline, in burst). So one client's ~1,200 MB/s is a limit on the client path, and **this configuration's disk path is about 1,900 MB/s.** The transfer size was already ruled out. **Which client resource sets it is still unidentified** -- not concurrency: outstanding I/O stays at 8.0. [The measurement](../ja/verification/throughput-iops-concurrency.md#2-台で測った結果--天井はクライアント側2026-09-19) (Japanese) | Done |
