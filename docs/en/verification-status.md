...
| What holds the high point at about 1,250 MB/s | **One candidate is closed (2026-09-18).** With `tcp_max_xfer_size` raised to 1048576 and the mount at `rsize=wsize=1048576`, the two points read 300.95 and 1,215.06 MB/s -- **4.04x**. Both ends rise (+17% and +13%) but **the ratio does not move**, so **the transfer size is not why 7.14x is absent**. **The ceiling on the high point is still unidentified** (`DiskIopsUtilization` is 34% there). [The re-measurement](../ja/verification/throughput-iops-concurrency.md#転送サイズ-1-mib-での再測定2026-09-18-倍率の不変) (Japanese) | Partly done; one file system for the rest |
