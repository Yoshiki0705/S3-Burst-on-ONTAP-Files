...
| Files copied to S3 | The target set is settled and **the freshness requirement is met by the job interval** (the floor on that interval is set by the transfer service; [DataSync cannot be scheduled faster than 1 hour](https://docs.aws.amazon.com/datasync/latest/userguide/task-scheduling.html)) | Freshness shorter than the job interval is needed, the volume is large, deletions have to propagate |
