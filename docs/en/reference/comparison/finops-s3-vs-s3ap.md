...
The following is generated by `tools/finops_model.py`. Regenerate this block whenever a unit price
changes.
...
| One origin for permissions | An audit can answer "who can read this" from one basis. Both the S3 AP side and the ONTAP side have to allow it | Effort in review cycles. Does not appear on the invoice |
...
| Recalculating | `python3 tools/finops_model.py --write` |
