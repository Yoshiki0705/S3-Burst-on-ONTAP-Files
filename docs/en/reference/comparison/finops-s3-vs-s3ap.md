...
### FlexClone is a third efficiency mechanism

Deduplication and compression fold identical byte sequences into one. FlexClone **adds references to
the same byte sequence**. It is not in the model below, but where collected data gets duplicated as
part of an operation, this is what moves the money.

Measured 2026-08-26, ap-northeast-1, ONTAP 9.18.1P3D1, on a UNIX volume with an S3 Access Point
attached:

| Point | Logical used | Physical used |
|---|---|---|
| Just after one 256 MiB file was PUT over S3 | — | 253,534,208 B |
| After cloning that file four times at file granularity | 1,350,942,720 B | 277,200,896 B |

Logical grew to five copies; physical grew by about 23 MB. **FSx for ONTAP bills provisioned SSD
capacity, so there is no need to provision capacity per duplicate.**

Doing the same on S3 requires `CopyObject`, each duplicate carries full storage charges, and the copy
requests are billed on top. **That difference is structural, and a unit rate does not close it.**

The other side, at the same granularity.

| Constraint | What it means |
|---|---|
| Changes written to the clone are not shared | NetApp states that copies consume no storage except what is required for metadata until changes are written to the copy ([FlexClone volumes, files, and LUNs](https://docs.netapp.com/us-en/ontap/concepts/flexclone-volumes-files-luns-concept.html)). Whatever is written consumes new blocks. A newly created clone uses about 0.5% of its nominal size for metadata ([determining space used](https://docs.netapp.com/us-en/ontap/volumes/determine-space-used-flexclone-task.html)) |
| A split ends the sharing | The explicit operation that turns a clone into an ordinary FlexVol (`volume clone split start`). **From ONTAP 9.4, on AFF systems where the volume guarantee is `none`, the split shares the physical blocks and does not copy the data, and space efficiency is preserved.** After the split, however, both the parent and the clone require the full space allocation set by their volume guarantees. Check the space needed beforehand with `volume clone show -estimate` ([splitting procedure](https://docs.netapp.com/us-en/ontap/volumes/split-flexclone-from-parent-task.html)). **Not measured on this path** (documented) |
| Referenced blocks are not released | Deleting data on the parent does not return capacity while a clone or a snapshot still references it |
| The bill does not move unless provisioning comes down | The saving appears only as capacity you did not have to provision. Where headroom is already generous, the invoice does not change |
| Teardown gains an ordering constraint | Delete the clone before the parent. While it sits in ONTAP's volume recovery queue the parent keeps `clone.has_flexclone` at `true`, and the AWS-side `delete-volume` silently reverts ([interoperability](../limits/s3ap-interoperability.md)) |
| A volume-granularity clone has a wait before it can be used | Attaching an access point to the clone needs an `fsvol-` identifier, and a volume created through the ONTAP API took 599 to 1,177 s to appear on the AWS side. A file-granularity clone lands in the same volume and has no such wait |
...
| A verification copy costs no capacity and is still readable over S3 | A file-granularity clone appears as a new key on the same access point; a volume-granularity clone can carry its own access point and its own policy. **Both share blocks** (measured: logical 1,350,942,720 B against physical 277,200,896 B) | No capacity provisioned per duplicate. The same thing on S3 charges storage for every duplicate plus the copy requests. Detail in [FlexClone is a third efficiency mechanism](#flexclone-is-a-third-efficiency-mechanism) |
| A duplicate can be given its own authorization boundary | A volume-granularity clone takes a separate access point, so its identity and policy are separate from the parent's. A write to the clone does not appear on the parent (measured) | Does not appear on the invoice. Building the same separation on S3 means copying the bytes into another bucket or prefix |
