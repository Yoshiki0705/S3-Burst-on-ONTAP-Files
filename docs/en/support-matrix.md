...
| Windows identity | An AD join is not required. Where a domain is not available, use a workgroup-mode CIFS server ([procedure](https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/smb-server-workgroup-setup.html); NTLM only, no Kerberos). **Creating one over REST is measured (2026-09-19, ONTAP 9.18.1P6)**: `POST /api/protocols/cifs/services` with `{"svm":{"name":...},"name":"SMBSRV1","workgroup":"WORKGROUP","enabled":true}` brings up an `enabled=true` CIFS server with no AD. **The same run confirmed that `is_multichannel_enabled` defaults to `false`.** **Multichannel itself was then measured coming up with no AD (2026-09-20, ONTAP 9.18.1P6, second generation at 1,536 MBps)**: with `is_multichannel_enabled=true` server side (`max_connections_per_session=32`), a Windows Server 2022 client connecting as a local ONTAP user over NTLM negotiated **SMB 3.1.1 and established `CurrentChannels` of 4** (one RSS-capable ENA interface, `ConnectionCountPerRssNetworkInterface=4`). **Measuring Multichannel therefore needs no directory.** The local user is prefixed with the **CIFS server** name rather than the SVM name (`SMBWG1\bench`), so adding it to a group as `v18c-smb-svm\bench` fails with `Failed to resolve name`. TCP 445 has to be opened separately as a prerequisite, and that must not be tied to whether a directory exists **If AD is joined**, every data operation then needs reachability to an AD domain controller, and `HeadBucket` succeeds even when AD is unreachable, so it cannot be used as a connectivity check |
