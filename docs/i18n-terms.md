...
日本語が原文で、英語はその訳文です。訳すのは Tier 1 のみです（[i18n-manifest.txt](i18n-manifest.txt)）。
...
| 原文 / 訳文 | source text / translation | 言語版の関係。**データ側には使わない。** かつて「正典」と呼んでいたが、宗教・文学の文脈が強い語であるうえ、同じ語をデータ側にも使っていて混同を招いた。`正本データ`（データ）と `基準構成`（構成）のどちらとも語がぶつからない |
...
Tier 1 の各文書は、食い違ったときにどちらが優先するかを対称に述べます。

- 日本語版は、自分が原文であり、技術的な正確さについて優先すると述べる
- 他言語版は、日本語版が原文で優先すること、食い違いを見つけたら報告してほしいと述べる

英語版の本文は `authoritative version` と述べています。同じ関係を反対側から言ったもので、英語では `authoritative` が一義的なため置き換えていません。
