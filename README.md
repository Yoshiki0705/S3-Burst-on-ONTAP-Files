...
> 収集側は S3 API のまま、利用側は NFS / SMB のままにできます。**ただし S3 に対応していれば動く
> とは限りません。** アクセスポイントの ARN または alias を扱えるか、必要なオペレーションが対応表に
> あるかは個別に確認します。
>
...
（言語版の関係を指す「原文」とは別の語です。日本語版が原文であるという記述は多言語ポリシーの節にあります。）
...
向きによって効く数値が変わります。**A の数値はこの構成での実測、B の数値は AWS のドキュメント記載**で、
性質が違うので同じ表に並べません。

- **収集を S3 API で行い、利用側が読み取りだけ**なら、効くのは取り込み側です。A は同一ボリュームで
  p50 9 ms（64 B、`actimeo=0`、n=30）、B は通常数秒です。
- **A は逆向きも測っています。** ファイル側で書いて S3 Access Point から読む向きは同一条件で
  p50 44 ms でした。取り込み側より遅いものの、どちらもミリ秒の範囲に収まります。
- **利用側が書き戻す設計にすると、B では約 60 秒が下流の鮮度になります。** これは待ち時間ではなく
  書き込みの無活動時間で、追記が続いている間はバケットに出ません。
- A 側の注意は既定マウントで、サーバ側の反映よりクライアントのキャッシュ期限が支配的になります。
  上の実測は `actimeo=0` での値です。

反映レートの上限、バージョニングの必須要件、競合時の扱いを含む B の仕様は
[S3 Files を選ぶ場合の仕様](docs/ja/reference/comparison/finops-s3-vs-s3ap.md#s3-files-を選ぶ場合の仕様)に、
A の測定条件は[検証状況](docs/ja/verification-status.md)にあります。
...
2. **[いまの構成から読む](docs/ja/reference/decision-trees/from-your-current-setup.md)（5 分）** —
   いまお使いの構成が当てはまるか。**当てはまらない形が 2 つあり、そこで終わってよいと書いてあります**
...
日本語が原文です。文書の言語はディレクトリで表します（`docs/ja/` / `docs/en/`）。
ファイル名に `.en` は付けません。
...
本リポジトリの日本語版が原文です。英語版は機械支援による訳文で、
