...
.PHONY: help lint markdown python format-python cfn i18n-check switcher-check switcher-write blog-sync ja-headings sources-export superseded \
...
diagram-flow: ## Diagrams must read rightwards and downwards, with labels under their icons
...
# Deliberately outside `all`. The drafts live under .private/, which is gitignored, so including them
# by default would fail here and pass on a runner that has no such directory -- the shape of every
# hole this repository has found in its own gates. Run it before publishing a draft; the tool says so
# loudly when the directory is absent rather than reporting a clean scan of nothing.
audit-drafts: ## Audit the article drafts under .private/ as well (run before publishing)
	@$(PY) tools/audit_public_output.py --include-drafts
...
all: lint i18n-check switcher-check xlang drift external-anchors incoming-probes outgoing-probes audit ja-headings secrets pinning zizmor links budget en-lang counts superseded blog-sync pattern-status iac-security finops diagram-fonts diagram-flow test commit-subjects ## Commit gate
